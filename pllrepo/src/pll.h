...
#define PLL_MAX_TIP_EV                                                         \
    0.999999999 /* max tip vector value, sum of EVs needs to be smaller        \
                   than 1.0, otherwise the numerics break down */
#define PLL_MAX_LOCAL_SMOOTHING_ITERATIONS                                     \
    32 /** @brief maximum iterations of smoothings per insert in the */
...
/* 0.020000
 * 0.00000000000000000000000000000044136090435925743185910935350715027016962154188875
 */
/* 0.030000
 * 0.00000000000000000000476844846859006690412039180149775802624789852441798419292220
 */
/* 0.040000
 * 0.00000000000000049522423236954066431210260930029681736928018820007024736185030633
 */
/* 0.050000
 * 0.00000000000050625351310359203371872643495343928538368616365517027588794007897377
 */
/* 0.060000
 * 0.00000000005134625283884191118711474021861409372524676086868566926568746566772461
 */
/* 0.070000
 * 0.00000000139080650074206434685544624965062437960128249869740102440118789672851562
 */
/* 0.080000
 * 0.00000001650681201563587066858709818343436959153791576682124286890029907226562500
 */
/* 0.090000
 * 0.00000011301977332931251259273962858978301859735893231118097901344299316406250000
 */
/* 0.100000
 * 0.00000052651925834844387815526344648331402709118265192955732345581054687500000000
 */
...
/*
   ...
   TO AVOID NUMERICAL PROBLEMS WHEN FREQ == 0 IN PARTITIONED MODELS, ESPECIALLY
   WITH AA previous value of FREQ_MIN was: 0.000001, but this seemed to cause
   problems with some of the 7-state secondary structure models with some rather
   exotic small toy test datasets, on the other hand 0.001 caused problems with
   some of the 16-state secondary structure models

   For some reason the frequency settings seem to be repeatedly causing
   numerical problems
*/

#define PLL_ITMAX 100 /* max number of iterations in brent's algorithm */

#define PLL_SHFT(a, b, c, d)                                                   \
    (a) = (b);                                                                 \
    (b) = (c);                                                                 \
    (c) = (d);
...
#define PLL_SWAP_PTR(x, y)                                                     \
    do {                                                                       \
        char *_t = x;                                                          \
        x = y;                                                                 \
        y = _t;                                                                \
    } while (0)
#define PLL_SWAP_INT(x, y)                                                     \
    do {                                                                       \
        int _t = x;                                                            \
        x = y;                                                                 \
        y = _t;                                                                \
...
#define PLL_POINT_GAMMA(prob, alpha, beta)                                     \
...
#define PLL_MIN_RECOM_FRACTION                                                 \
    0.1 /* at least this % of inner nodes will be allocated in RAM */
#define PLL_MAX_RECOM_FRACTION                                                 \
    1.0 /* always 1, just there for boundary checks */
...
typedef struct {
    ...
    int *stlen; /**< Number of tips behind the current orientation of the
                   indexed inner node (subtree size/cost) */
    int *unpinnable; /**< size:numVectors , TRUE if we dont need the vector */
    int maxVectorsUsed;
    pllBoolean allSlotsBusy; /**< on if all slots contain an ancesctral node
                                (the usual case after first full traversal) */
} recompVectors;
/* E recomp */
...
/** @brief Per-site Rate category entry: likelihood per-site and CAT rate
 * applied ???
 *
 */
 ...
 *
 * Contains the information required to execute an operation in a step of the
 * tree traversal. q   r \ /
 *   p
 *
 * The entry defines 2 input/parent nodes (q and r) and one output/child node
 * (p) qz represents the branch length(s) of the branch connecting q and p rz
 * represents the branch length(s) of the branch connecting r and p PLL_TIP_TIP
 * Both p and r are tips PLL_INNER_INNER Both p and r are inner nodes
 * @note PLL_TIP_INNER   q is a tip and r is an inner node (by convention, flip
 * q and r if required)
 */
typedef struct {
    int tipCase; /**< Type of entry, must be PLL_TIP_TIP PLL_TIP_INNER or
                    PLL_INNER_INNER */
    ...
    /* recom */
    int slot_p; /**< In recomputation mode, the RAM slot index for likelihood
                   vector of node p, otherwise unused */
    int slot_q; /**< In recomputation mode, the RAM slot index for likelihood
                   vector of node q, otherwise unused */
    int slot_r; /**< In recomputation mode, the RAM slot index for likelihood
                   vector of node r, otherwise unused */
                /* E recom */
} traversalInfo;
 ...
 *
 * Each inner node is a trifurcation in the tree represented as a circular list
 * containing 3 node records. One node record uniquely identifies a subtree, and
 * the orientation of the likelihood vector within a node
 *
 ...
 * the data structure below is fundamental for representing trees
   ...
   Inner nodes are represented by three instances of the nodeptr data structure
 that is linked via a cyclic list using the next pointer.
   ...
   Note that the node number is important for indexing tip sequence data as well
 as inner likelihood vectors and that it is this number (the index) that
 actually gets stored in the traversal descriptor.

   Tip nodes are non-cyclic nodes that simply consist of one instance/allocation
 of nodeptr.
   ...
   now let's assume that we want to build a four taxon tree with tips t1, t2,
 t3, t4 and inner nodes (p1,p2,p3) and (q1,q2,q3).
   ...
   now we need to link the nodes together such that they form a tree, let's
 assume we want ((t1,t2), (t3, t4));
   ...
   the above operation for connecting nodes is implemented in functions hookup()
 which will set the back pointers of two nodes that are to be connected as well
 as the branch lengths.

   The branchInfo data field is a pointer to a data-structure that stores
 meta-data and requires the tree not to change while it is being used.

   Also, this pointer needs to be set by doing a full tree traversal on the
 tree.

   Note that q1->bInf == t3->bInf in the above example.

   The hash number is used for mapping bipartitions to a hash table as described
 in the following paper:

   A. Aberer, N. Pattengale, A. Stamatakis: "Parallelized phylogenetic
 post-analysis on multi-core architectures". Journal of Computational Science 1,
 107-114, 2010.

   The support data field stores the support value for the branch associated
 with each nodeptr structure. Note that support always refers to branches.
   ...
   parsimony, bipartition, or likelihood vector at a node with respect to the
 virtual root.

   Essentially, they are just used as single presence/absence bits and ONLY for
 inner nodes!

   When setting up new inner nodes, one of the three pointers in the cyclic list
 must have x = 1 and the other two x = 0;
   ...
   This would mean that the virtual root is located at the inner branch of the
 four taxon tree ((t1,t2),(t3,t4));

   When we re-root the tree at some other branch we need to update the location
 of the x pointer that is set to 1.
   ...
   When we re-locate the root to branch p1 <-> t1 the fact that we have to
 "rotate" the x value that is set to 1 to another node of the cyclic list
 representing the abstract topological node p, also tells us that we need to
 re-compute the conditional likelihood array for p.

   Note that, only one likelihood or parsimony array is stored per inner node
 and the location of x essentially tells us which subtree it summarizes, if
 p1->x == 1, it summarizes subtree (t2, (t3, t4)), if p3->x = 1 the likelihood
 vector associated with node p summarizes subtree (t1, t2).

   @todo I think we should rename the back pointer. It's not back, it can be
 forward depending on the orientation. We should renmae it to outer. Back is too
 confusing, I would assume it's the opposite of next, i.e. previous.

   @struct noderec

   @brief Tree node record

   A node in a tree is a structure which contains a cyclic list of pointers to 3
 nodes which we call a \e roundabout. The first node is the structure itself,
 and the other two nodes are accessed via \a noderec->next and \a
 noderec->next->next. To access the outer node with which each of the 3 nodes
 forms an edge one has to use the \a back pointer
   ...
   In general, tips (i.e. leaves) are numbered from 1 to \e n where \e n is the
 number of taxa. Identifiers for internal nodes start from \e n + 1. Note that
 for a given inner node, the identifier must be the same for all 3 nodes that
 compose it.
...
/* @brief Alignment, transition model, model of rate heterogenety and likelihood
  vectors for one partition.
  *
  ...
  * Pij(t) is the probability of going from state i to state j in a branch of
  length t
  * Relative substitution rates (Entries in the Q matrix)
  * In GTR we can write Q = S * D, where S is a symmetrical matrix and D a
  diagonal with the state frequencies
...
/** @struct pInfo
    ...
    This data structure encapsulates all properties and auxiliary variables that
   together consist a partition.
    ...
    This is the total number of possible states that can appear in the
   alignment. This includes degenerate (undetermined) bases
    ...
    @var pInfo::lower
    @brief Position of the first site in the alignment that is part of this
   partition [1, tr->originalCrunchedLength]

    @var pInfo::upper
    @brief Position of the last site that is part of this partition plus one
   (i.e. position of the first site that is not part of this partition)
    ...
    Number of times this particular site appeared in the partition before the
   duplicates were removed and replaced by this weight
    ...
    In GTR we can write \f$ Q = S * D \f$, where \f$ S \f$ is a symmetrical
   matrix and \f$ D \f$ a diagonal with the state frequencies, which is
   represented by the array \a frequencies. The symmetrical matrix is the array
   \a substRates

    @var pInfo::frequencies
    @brief State frequencies, entries in D are initialized as
   ...
   \a substRates
    ...
    @var pInfo::tipVector
    @brief Precomputed (based on current P matrix) conditional likelihood
   vectors for every possible base
    ...
    @brief Precomputed (based on current P matrix) conditional likelihood
   vectors for every possible base for the LG4 model
    ...
    In case \a pInfo::dataType is set to \a PLL_AA_DATA then \a protModels
   indicates the index in the global array \a protModels of the protein model
   that the current partition uses.

    @var pInfo::autoProtModels
    @brief Best fitted protein model for the \b PLL_AUTO partitions

    If \a protModels is set to \b PLL_AUTO then \a autoProtModels holds the
   currently detected best fitting protein model for the partition
...
typedef struct {
    ...
    /* MODEL OF RATE HETEROGENETY, We use either GAMMA or PSR */
    /* Rate heterogenety: Per Site Categories (PSR) model aka CAT, see
     * updatePerSiteRates() */
    /* Rate of site i is given by perSiteRates[rateCategory[i]] */
    double *perSiteRates;
    int *rateCategory;
    ...
    /* In GTR we can write Q = S * D, where S is a symmetrical matrix and D a
     * diagonal with the state frequencies */
    double *substRates;  /**< TRANSITION MODEL Entries in S, e.g. 6 free
                            parameters in DNA */
    double *frequencies; /**< State frequencies, entries in D, are initialized
                            as empiricalFrequencies */
    double *freqExponents;
    /* Matrix decomposition: @todo map this syntax to Explanation of the
    ...
    int protUseEmpiricalFreqs; /**< Whether to use empirical frequencies for
                                  protein model */

    pllBoolean nonGTR;
    pllBoolean
        optimizeBaseFrequencies; /**< Whether to optimize base frequencies */
    pllBoolean optimizeAlphaParameter; /**< Whether to optimize alpha parameters
                                          and gamma rates */
    pllBoolean optimizeSubstitutionRates; /**< Whether to optimize substitution
                                             rates */
    int *symmetryVector; /**< Specify linkage between substitution rate
                            parameters */
    int *frequencyGrouping;

    /* LIKELIHOOD VECTORS */

    /* partial LH Inner vectors  ancestral vectors, we have 2*tips - 3 inner
     * nodes */
    double **xVector; /**< Conditional likelihood vectors for inner nodes */
    unsigned char **yVector; /**< Tip entries (sequence) for tip nodes */
    unsigned int
        *globalScaler; /**< Counters for scaling operations done at node i */
    ...
    int **
        expVector; /**< @brief An entry per inner node. Each element is an array
                      of size the number of sites in the current partition and
                      represents how many times the respective site has been
                      scaled in the subtree rooted at the current node */
    size_t *expSpaceVector; /**< @brief Each entry represents an inner node and
                               states the size of the corresponding element in
                               \a expVector, which is the number of sites for
                               the current partition */

    /* These are for the saveMemory option (tracking gaps to skip computations
     * and memory) */
    size_t *xSpaceVector;    /* Size of conditional likelihood vectors per inner
                                node */
    int gapVectorLength;     /** Length of \a gapVector bitvector in unsigned
                                integers assuming that \a unsigned \a int is 32bits.
                                It is set to partition size / 32 */
    unsigned int *gapVector; /** A bit vector of size \a gapVectorLength * 32
                                bits. A bit is set to 1 if the corresponding */
    double *gapColumn;
    ...
    // This array is necessary in case the pll alignment is not sorted
    // decreasingly by score
    ...
    /* This buffer of size width is used to store the ancestral state at a node
     * of the tree. */
...
/** @brief Tree topology.
 *
 * @todo Apart from the topology this structure contains several fields that act
 * like global variables in raxml
 */
typedef struct {
    ...
    /* recomp */
    recompVectors *
        rvec; /**< this data structure tracks which vectors store which nodes */
    float maxMegabytesMemory;  /**< User says how many MB in main memory should
                                  be used */
    float vectorRecomFraction; /**< vectorRecomFraction ~= 0.8 *
                                  maxMegabytesMemory  */
    pllBoolean
        useRecom; /**< ON if we apply recomputation of ancestral vectors*/
#ifdef _DEBUG_RECOMPUTATION
    ...
    double *lhs;    /**< Array to store per-site log likelihoods of \a
                       originalCrunchedLength (compressed) sites */
    ...
    pllBoolean
        grouped; /**< No idea what this is, but is always set to PLL_FALSE */
    pllBoolean constrained; /**< No idea what this is, but is always set to
                               PLL_FALSE */
    int threadID;
    ...
    /* model stuff end */
    unsigned char *
        *yVector; /**< list of raw sequences (parsed from the alignment)*/

    int secondaryStructureModel;
    int originalCrunchedLength; /**< Length of alignment after removing
                                   duplicate sites in each partition */
    ...
    double startLH;
    double endLH;
    double likelihood; /**< last likelihood value evaluated for the current
                          topology */
    ...
    unsigned int bestScoreThisIter;
    ...
    nodeptr insertNode; /**< the node where insertion should take place .
                           Together with \a removeNode represents an SPR move*/
    ...
    checkPointState ckp;
    pllBoolean
        thoroughInsertion; /**< true if the neighbor branches should be
                              optimized when a subtree is inserted (slower)*/
    ...
    int initial; /**< user defined rearrangement radius which also sets bestTrav
                    if initialSet is set */
    pllBoolean initialSet; /**< set bestTrav according to initial */
    int mode;              /**< candidate for removal */
    pllBoolean perGeneBranchLengths;
    pllBoolean
        permuteTreeoptimize; /**< randomly select subtrees for SPR moves */
...
/** @brief small helper data structure for printing out/downstream use of
 * marginal ancestral probability vectors.
 *
 * it is allocated as an array that has the same length as the input alignment
 * and can be used to index the ancestral states for each position/site/pattern
 *   */
typedef struct {
...
typedef struct {
    ...
    int evLength;
    int eiLength;
    int substRatesLength;  /**< (s^2 - s)/2 free model parameters for matrix Q
                              i.e. substitution rates */
...
/** @struct pllRearrangeAttr
    ...
    @brief Structure holding attributes for searching possible tree
   rearrangements
    ...
    @var pllRearrangeAttr:mintrav
      The minimum radius around the origin node \a p for which nodes should be
   tested

    @var pllRearrangeAttr:maxtrav
      The maximum radius around the origin node \a p for which nodes should be
   tested
...
/** @typedef pllRearrangeInfo
    ...
    @var pllRearrangeInfo::rearrangeType
      Type of rearrangement. Can be \b PLL_REARRANGE_SPR, \b PLL_REARRANGE_NNI
   or \b PLL_REARRANGE_TBR
...
typedef struct {
    ...
    int originalSeqLength; /**< @brief Original length of sequences (not
                              modified after removing duplicates) */
    char **sequenceLabels; /**< @brief An array of where the \a i-th element is
                              the name of the \a i-th sequence */
    unsigned char **sequenceData; /**< @brief The actual sequence data */
    int *siteWeights; /**< @brief An array where the \a i-th element indicates
                         how many times site \a i appeared (prior to duplicates
                         removal) in the alignment */
} pllAlignmentData;
 ...
 * @param useRecom       If set to \b PLL_TRUE, enables ancestral state
 * recomputation
 *
 * @todo                 Document fastScaling, rate heterogeneity and saveMemory
 * and useRecom
 *
 * @note                 Do not set \a saveMemory to when using \a useRecom as
 * memory saving techniques are not yet implemented for ancestral state
 * recomputation.
 *
 * @return               On success returns an instance to PLL, otherwise \b
 * NULL
 */
 ...
 */
extern int pllLoadAlignment(pllInstance *tr, pllAlignmentData *alignmentData,
                            partitionList *pList);
 ...
 * @return   A list of \a pl->numberOfPartitions arrays each of size
             \a pl->partitionData[i]->states, where \a i is the \a i-th
 partition
*/
extern double **pllBaseFrequenciesAlignment(pllAlignmentData *alignmentData,
                                            partitionList *pl);
...
extern void pllSetBranchLength(pllInstance *tr, nodeptr p, int partition_id,
                               double bl);
extern int pllNniSearch(pllInstance *tr, partitionList *pr, int estimateModel);
extern void pllOptimizeBranchLengths(pllInstance *tr, partitionList *pr,
                                     int maxSmoothIterations);

extern void pllEvaluateLikelihood(pllInstance *tr, partitionList *pr, nodeptr p,
                                  pllBoolean fullTraversal,
                                  pllBoolean getPerSiteLikelihoods);
extern void pllUpdatePartials(pllInstance *tr, partitionList *pr, nodeptr p,
                              pllBoolean masked);
extern void pllUpdatePartialsAncestral(pllInstance *tr, partitionList *pr,
                                       nodeptr p);
extern void pllNewviewIterative(pllInstance *tr, partitionList *pr,
                                int startIndex);
extern void pllEvaluateIterative(pllInstance *tr, partitionList *pr,
                                 pllBoolean getPerSiteLikelihoods);
...
extern char *pllTreeToNewick(char *treestr, pllInstance *tr, partitionList *pr,
...
partitionList *pllPartitionsCommit(pllQueue *parts,
                                   pllAlignmentData *alignmentData);
int pllPartitionsValidate(pllQueue *parts, pllAlignmentData *alignmentData);
extern void pllAlignmentRemoveDups(pllAlignmentData *alignmentData,
                                   partitionList *pl);
...
extern pllAlignmentData *pllParsePHYLIPString(const char *rawdata,
                                              long filesize);
...
int pllSetSubstitutionRateMatrixSymmetries(char *string, partitionList *pr,
                                           int model);
void pllSetFixedAlpha(double alpha, int model, partitionList *pr,
                      pllInstance *tr);
void pllSetFixedBaseFrequencies(double *f, int length, int model,
                                partitionList *pr, pllInstance *tr);
int pllSetOptimizeBaseFrequencies(int model, partitionList *pr,
                                  pllInstance *tr);
void pllSetSubstitutionMatrix(double *q, int length, int model,
                              partitionList *pr, pllInstance *tr);
void pllSetFixedSubstitutionMatrix(double *q, int length, int model,
                                   partitionList *pr, pllInstance *tr);
int pllGetInstRateMatrix(partitionList *pr, int model, double *outBuffer);
int pllOptimizeModelParameters(pllInstance *tr, partitionList *pr,
                               double likelihoodEpsilon);
double pllGetAlpha(partitionList *pr, int pid);
void pllGetGammaRates(partitionList *pr, int pid, double *outBuffer);
extern void pllGetBaseFrequencies(partitionList *pr, int model,
                                  double *outBuffer);
extern void pllGetSubstitutionMatrix(partitionList *pr, int model,
                                     double *outBuffer);
void pllEmpiricalFrequenciesDestroy(double ***empiricalFrequencies, int models);
extern void pllOptRatesGeneric(pllInstance *tr, partitionList *pr,
                               double modelEpsilon, linkageList *ll);
extern void pllOptBaseFreqs(pllInstance *tr, partitionList *pr,
                            double modelEpsilon, linkageList *ll);
extern void pllOptAlphasGeneric(pllInstance *tr, partitionList *pr,
                                double modelEpsilon, linkageList *ll);
...
extern void pllMakeParsimonyTreeFast(pllInstance *tr, partitionList *pr,
                                     int sprDist);
void pllComputeRandomizedStepwiseAdditionParsimonyTree(
    pllInstance *tr, partitionList *partitions, int sprDist);
...
extern void pllGetAncestralState(pllInstance *tr, partitionList *pr, nodeptr p,
                                 double *outProbs, char *outSequence);
unsigned int pllEvaluateParsimony(pllInstance *tr, partitionList *pr, nodeptr p,
                                  pllBoolean full, pllBoolean perSiteScores);
void pllInitParsimonyStructures(pllInstance *tr, partitionList *pr,
                                pllBoolean perSiteScores);
...
void pllRearrangeSearch(pllInstance *tr, partitionList *pr, int rearrangeType,
                        nodeptr p, int mintrav, int maxtrav,
                        pllRearrangeList *bestList);
void pllRearrangeCommit(pllInstance *tr, partitionList *pr,
                        pllRearrangeInfo *rearr, int saveRollbackInfo);
int pllRearrangeRollback(pllInstance *tr, partitionList *pr);
void pllClearRearrangeHistory(pllInstance *tr);
int pllRaxmlSearchAlgorithm(pllInstance *tr, partitionList *pr,
                            pllBoolean estimateModel);
int pllGetTransitionMatrix(pllInstance *tr, partitionList *pr, nodeptr p,
                           int model, int rate, double *outBuffer);
void pllGetTransitionMatrix2(pllInstance *tr, partitionList *pr, int model,
                             nodeptr p, double *outBuffer);
int pllGetCLV(pllInstance *tr, partitionList *pr, nodeptr p, int partition,
              double *outProbs);
