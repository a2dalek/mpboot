...
// Add site scores in q and r to p
// q and r are children of p
void addPerSiteSubtreeScores(partitionList *pr, int pNumber, int qNumber,
                             int rNumber) {
    //	parsimonyNumber * pBuf, * qBuf, *rBuf;
    //	for(int i = 0; i < pr->numberOfPartitions; i++){
...
//        return;
        ...
        if (globalParam->sankoff_short_int) {
            ...
            case 4:
                newviewSankoffParsimonyIterativeFastSIMD<
                    Vec16us, parsimonyNumberShort, 4>(tr, pr);
                break;
            case 5:
                newviewSankoffParsimonyIterativeFastSIMD<
                    Vec16us, parsimonyNumberShort, 5>(tr, pr);
                break;
            case 20:
                newviewSankoffParsimonyIterativeFastSIMD<
            ...
            switch (pr->partitionData[0]->states) {
            case 4:
                newviewSankoffParsimonyIterativeFastSIMD<Vec8ui,
                                                         parsimonyNumber, 4>(
                    tr, pr);
                break;
            case 5:
        ...
        if (globalParam->sankoff_short_int) {
            ...
            switch (pr->partitionData[0]->states) {
            case 4:
                newviewSankoffParsimonyIterativeFastSIMD<Vec4ui,
                                                         parsimonyNumber, 4>(
                    tr, pr);
                break;
            case 20:
...
//        return _evaluateSankoffParsimonyIterativeFast(tr, pr, perSiteScores);
        ...
        if (globalParam->sankoff_short_int) {
            ...
            case 5:
                return evaluateSankoffParsimonyIterativeFastSIMD<
                    Vec16us, parsimonyNumberShort, 5, true>(tr, pr,
                                                            perSiteScores);
            case 20:
                return evaluateSankoffParsimonyIterativeFastSIMD<
        ...
        for (model = 0; model < pr->numberOfPartitions; model++) {
            ...
            */
            ...
            /*
               ...
               width
               * k]); right[k] = &(pr->partitionData[model]->parsVect[(width *
               states
        ...
        if (tr->usingSA) {
            if (mp < tr->bestParsimony) {
                ...
                if (random_double() <= probability) {
                    if (tr->printAcceptTree) {
                        std::streamsize ss = std::cout.precision();
                        std::cout.precision (8);
                        std::cout << "Accept worse tree with probability = " << probability 
                                  << ", current temperature = " << tr->temperature 
                                  << ", delta = " << delta << "\n";
...
static int rearrangeParsimony(pllInstance *tr, partitionList *pr, nodeptr p,
                              int mintrav, int maxtrav, pllBoolean doAll,
                              int perSiteScores) {
...
static bool isUnambiguous(char nucleotide, int dataType) {
    ...
    if (dataType == PLL_DNA_DATA)
        return (nucleotide == 1) || (nucleotide == 2) || (nucleotide == 4) ||
               (nucleotide == 8);
...
static void _pllMakeParsimonyTreeFast(pllInstance *tr, partitionList *pr,
    ...
    //	int * hill_climbing_perm = (int *)rax_malloc((size_t)(tr->mxtips +
    // tr->mxtips - 1) * sizeof(int));
    int j;
    //	makePermutationFast(hill_climbing_perm, tr->mxtips + tr->mxtips - 2,
    // tr);
...
void decreaseTemp(pllInstance *tr, int times = 1) {
...
void InitSA(pllInstance *tr) {
    tr->maxCoolingTimes = (tr->mxtips + 99) / 100 * 100 + 1;
    tr->coolingTimes = 100;
    tr->last_temp = tr->startTemp;
    tr->temperature = tr->startTemp;

    bool use_default_temp = false;
    if (tr->finalTemp <= 0 || tr->startTemp <= 0) {
        std::cout << "Temperature can not smaller than or equal to 0, use default temperature\n";
        use_default_temp = true;
    } 

    if (tr->startTemp <= tr->finalTemp) {
        std::cout << "Start temp can not be smaller than or equal to final temp, use default temperature\n";
        use_default_temp = true;
    }
...
void pllComputeSankoffPatternParsimony(pllInstance *tr, partitionList *pr,
                                       unsigned short *ptn_pars,
                                       int *cur_pars) {
    int ptn = 0;
    int sum = 0;
    ...
    for (int i = 0; i < pr->numberOfPartitions; i++) {
        Numeric *ptnScore =
            (Numeric *)pr->partitionData[i]->informativePtnScore;
...
void pllComputePatternParsimony(pllInstance *tr, partitionList *pr,
    ...
    if (pllCostMatrix) {
        if (globalParam->sankoff_short_int)
            return pllComputeSankoffPatternParsimony<parsimonyNumberShort>(
                tr, pr, ptn_pars, cur_pars);
        else
            return pllComputeSankoffPatternParsimony<parsimonyNumber>(
                tr, pr, ptn_pars, cur_pars);
    }
...
void testSiteParsimony(Params &params) {
    ...
    // params.intype); 	iqtree->setAlignment(&alignment);
    ...
    //	iqtree->pllAttr.numberOfThreads = 2; /* This only affects the pthreads
    // version */
    //
    ...
    //
    //	iqtree->pllAlignment = pllParseAlignmentFile(PLL_FORMAT_PHYLIP,
    // pllAln.c_str());
    //
    //	/* Read in the partition information */
    //	pllQueue *partitionInfo;
    //	ofstream pllPartitionFileHandle;
    //	string pllPartitionFileName = string(params.out_prefix) +
    //".pll_partitions";
    ...
    //	iqtree->pllPartitions = pllPartitionsCommit(partitionInfo,
    // iqtree->pllAlignment);
    //
    //	/* We don't need the the intermediate partition queue structure anymore
    //*/ 	pllQueuePartitionsDestroy(&partitionInfo);
    //
    //	/* eliminate duplicate sites from the alignment and update weights
    // vector */ 	pllAlignmentRemoveDups(iqtree->pllAlignment,
    // iqtree->pllPartitions);
    //
    //
    //	cout << "Finished initialization!" << endl;
    //
    //	/************************************ END: Initialization for PLL and
    // sNNI *************************************************/ 	pllNewickTree *
    // newick; 	newick = pllNewickParseString (iqtree->getTreeString().c_str());
    ...
    // iqtree->pllPartitions)) { 		printf("Incompatible
    // tree/alignment combination\n"); 		exit(1);
    //	}
    //
    //	int npatterns = iqtree->aln->getNPattern();
    //
    //	int i, j, uninformative_count = 0;
    //	pllInitParsimonyStructures (iqtree->pllInst, iqtree->pllPartitions,
    // PLL_TRUE);   // the last parameter is that you want to allocate buffers
    // for the per site computation
    //
    //	iqtree->pllInst->bestParsimony = UINT_MAX;
    //
    //	for (i = 0; i < iqtree->pllPartitions->numberOfPartitions; ++i)
    //	{
    //		for (j = 0; j < iqtree->pllPartitions->partitionData[i]->width;
    //++j){ 			if(!isInformative(iqtree->pllInst,
    // iqtree->pllPartitions->partitionData[i]->dataType, j))
    //				uninformative_count++;
    //		}
    //	}
    //	cout << "There are " << uninformative_count << " uninformative sites."
    //<< endl;
    //
    ...
    //	}
    //
    //	cout << endl << "sum_test = " << sum_test << ", zero_count = " <<
    // zero_count << endl;
    //
    //	cout << endl << "************** PARSIMONY SCORES BY IQTREE
    //**************************" << endl;
    //    pllTreeToNewick(iqtree->pllInst->tree_string, iqtree->pllInst,
    //    iqtree->pllPartitions, iqtree->pllInst->start->back,
    //            PLL_FALSE, PLL_TRUE, PLL_FALSE, PLL_FALSE, PLL_FALSE,
...
void computeUserTreeParsimomy(Params &params) {
    ...
    if (params.sankoff_cost_file) {
        ...
        if (ptree->pllPartitions->partitionData[0]->informativePtnWgt) {
            int diffCount = 0;
            unsigned int *ptnWgt =
                (unsigned int *)ptree->pllPartitions->partitionData[0]
                    ->informativePtnWgt;
...
/* Diep begin */
void pllSortedAlignmentRemoveDups(pllAlignmentData *alignmentData,
                                  partitionList *pl) {
    int i, j, k, p;
    char ***sites;
    void **memptr;
    ...
    for (p = 0; p < pl->numberOfPartitions; ++p) {
        sites[p] =
            (char **)rax_malloc(pl->partitionData[p]->width * sizeof(char *));
        memptr[p] = rax_malloc((alignmentData->sequenceCount + 1) *
                               pl->partitionData[p]->width * sizeof(char));

        for (i = 0; i < pl->partitionData[p]->width; ++i) {
            sites[p][i] =
                (char *)((char *)memptr[p] +
                         i * (alignmentData->sequenceCount + 1) * sizeof(char));
        }
        ...
        for (i = 0; i < pl->partitionData[p]->width; ++i) {
            for (j = 0; j < alignmentData->sequenceCount; ++j) {
                sites[p][i][j] =
                    alignmentData
                        ->sequenceData[j + 1][pl->partitionData[p]->lower + i];
            }
            sites[p][i][j] = 0;
        }
    ...
    alignmentData->sequenceData[0] = (unsigned char *)rax_malloc(
        (alignmentData->sequenceLength + 1) * sizeof(unsigned char) *
        alignmentData->sequenceCount);
    for (i = 0; i < alignmentData->sequenceCount; ++i) {
        alignmentData->sequenceData[i + 1] =
            (unsigned char *)(alignmentData->sequenceData[0] +
                              i * (alignmentData->sequenceLength + 1) *
                                  sizeof(unsigned char));
        alignmentData->sequenceData[i + 1][alignmentData->sequenceLength] = 0;
    }

    alignmentData->siteWeights =
        (int *)rax_malloc((alignmentData->sequenceLength) * sizeof(int));
...
void testSPROnUserTree(Params &params) {
    ...
    // ptree->setAlignment(ptree->aln); // IMPORTANT: Always call setAlignment()
    // after readTree()
    ptree->initializeAllPartialPars();
    ptree->clearAllPartialLH();
    ptree->initializePLL(params);
    ...
    nodeRectifierPars(ptree->pllInst);
    ptree->pllInst->removeNode = ptree->pllInst->insertNode = NULL;

    rearrangeParsimony(ptree->pllInst, ptree->pllPartitions,
                       ptree->pllInst->nodep[7], params.spr_mintrav,
                       params.spr_maxtrav, PLL_FALSE, 0);
    if (ptree->pllInst->bestParsimony < curScore) {
        restoreTreeRearrangeParsimony(ptree->pllInst, ptree->pllPartitions, 0);
    }
    pllTreeToNewick(ptree->pllInst->tree_string, ptree->pllInst,
                    ptree->pllPartitions, ptree->pllInst->start->back, PLL_TRUE,
                    PLL_TRUE, 0, 0, 0, PLL_SUMMARIZE_LH, 0, 0);
    string treeString = string(ptree->pllInst->tree_string);
    ptree->readTreeString(treeString);
    ptree->initializeAllPartialPars();
    ptree->clearAllPartialLH();
    curScore = ptree->computeParsimony();
    assert(curScore == ptree->pllInst->bestParsimony);
    out << "Tree after 1 SPR looks like: \n";
    ptree->sortTaxa();
    ptree->drawTree(out, WT_BR_SCALE, epsilon);
    out << "Parsimony score: " << ptree->pllInst->bestParsimony << endl;
    _pllFreeParsimonyDataStructures(ptree->pllInst, ptree->pllPartitions);
