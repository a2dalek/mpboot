...
/**
        ...
        @param error error message
 */
void outError(string error) { outError(error.c_str()); }
 ...
 */
void outWarning(const char *warn) { cerr << "WARNING: " << warn << endl; }

void outWarning(string warn) { outWarning(warn.c_str()); }
...
void readWeightFile(Params &params, int ntaxa, double &scale,
                    StrVector &tax_name, DoubleVector &tax_weight) {
    cout << "Reading scale factor and taxa weights file " << params.param_file
         << " ..." << endl;
    try {
        ifstream in;
...
void readInitTaxaFile(Params &params, int ntaxa, StrVector &tax_name) {
    cout << "Reading initial taxa set file " << params.initial_file << " ..."
         << endl;
...
void readInitAreaFile(Params &params, int nareas, StrVector &area_name) {
    cout << "Reading initial area file " << params.initial_area_file << " ..."
         << endl;
    readStringFile(params.initial_area_file, nareas, area_name);
}

void readAreasBoundary(char *file_name, MSetsBlock *areas,
    ...
    try {
        ...
        for (seq1 = 0; seq1 < nset; seq1++) {
            ...
            if (seq_name != areas->getSet(seq1)->name)
                throw "Area name " + seq_name + " is different from " +
                    areas->getSet(seq1)->name;
        ...
        for (seq1 = 0; seq1 < nset - 1; seq1++) {
            if (areas_boundary[seq1 * nset + seq1] <= 1e-6)
                throw "Diagonal elements of distance matrix should represent "
                      "the "
                      "boundary of single areas";
            for (seq2 = seq1 + 1; seq2 < nset; seq2++)
                if (areas_boundary[seq1 * nset + seq2] !=
                    areas_boundary[seq2 * nset + seq1])
                    throw "Shared boundary between " +
                        areas->getSet(seq1)->name + " and " +
                        areas->getSet(seq2)->name + " is not symmetric";
        }
...
void get2RandNumb(const int size, int &first, int &second) {
    ...
    // pick a random element from what's left (there is one fewer to choose
    // from)...
...
void parseArg(int argc, char *argv[], Params &params) {
    ...
    params.unsuccess_iteration_hclimb = -1;
    ...
    params.cutoff_percent =
        10; // Diep: Default = 10 for UFBoot-MP release;
            // Hidden test value: >100 for UFBoot logl-cutoff
    ...
    params.coolingSchedule = EXPONENTIAL_MULTIPLICATIVE_COOLING;
    params.start_temp = -1.0;
    params.final_temp = -1.0;
    params.pureSA = false;
    params.printAcceptTree = false;
    ...
    for (cnt = 1; cnt < argc; cnt++) {
        try {
            ...
            else if (strcmp(argv[cnt], "-mprob") ==
                     0) { // compute multinomial distribution probability
                cnt++;
                if (cnt >= argc)
            ...
            if (strcmp(argv[cnt], "-st") == 0) {
                cnt++;
                if (cnt >= argc)
                    throw "Use -st BIN or -st DNA or -st AA or -st CODON or "
                          "-st MORPH";
            ...
            if (strcmp(argv[cnt], "-starttree") == 0) {
                ...
                if (strcmp(argv[cnt], "BIONJ") == 0)
                    ...
                    throw "Invalid option, please use -starttree with BIONJ or "
                          "PARS or "
                          "PLLPARS";
                continue;
            }
            ...
            if (strcmp(argv[cnt], "-pdel") == 0 ||
                ...
                if (params.p_delete < 0.0 || params.p_delete > 1.0)
                    throw "Probability of deleting a leaf must be between 0 "
                          "and 1";
                continue;
            }
            ...
            if (strcmp(argv[cnt], "-sr") == 0) {
                ...
                if (params.max_iterations <= params.min_iterations)
                    throw "Specified max iteration must be greater than min "
                          "iteration";
                continue;
            }
            ...
            if (strcmp(argv[cnt], "-lbp") == 0) {
                ...
                if (params.localbp_replicates < 1000 &&
                    params.localbp_replicates != 0)
                    throw "Local bootstrap (LBP) replicates must be at least "
                          "1000";
                continue;
            }
            ...
            if (strcmp(argv[cnt], "-k%") == 0) {
                ...
                if (cnt >= argc)
                    throw "Use -k% <k in %>";
                // convert_range(argv[cnt], params.k_percent, params.sub_size,
                // params.step_size);
            ...
            if (strcmp(argv[cnt], "-bb") == 0) {
                ...
                params.stop_condition =
                    SC_UNSUCCESS_ITERATION; // Diep: because MP already has
                                            // refinement
            ...
            if (strcmp(argv[cnt], "-topboot") == 0) {
                cnt++;
                if (cnt >= argc)
                    throw "Use -topboot <number of top trees stored for each "
                          "boot "
                          "sample>";
            ...
            if (strcmp(argv[cnt], "-ratchet_iter") == 0) {
                cnt++;
                if (cnt >= argc)
                    throw "Use -ratchet_iter <number iterations between 2 "
                          "ratchets>";
            ...
            if (strcmp(argv[cnt], "-ratchet_wgt") == 0) {
                cnt++;
                if (cnt >= argc)
                    throw "Use -ratchet_wgt <weight to add to each resampled "
                          "character>";
            ...
            if (strcmp(argv[cnt], "-ratchet_percent") == 0) {
                cnt++;
                if (cnt >= argc)
                    throw "Use -ratchet_percent <percentage of characters in "
                          "subset "
                          "picked for ratchet>";
            ...
            if (strcmp(argv[cnt], "-short_off") == 0) {
                params.sankoff_short_int =
                    false; // To turn off using short for pattern parsimony
                continue;
            }
            ...
            if (strcmp(argv[cnt], "-print_sa") == 0) {
                params.printAcceptTree = true;
                continue;
            }
            ...
            if (strcmp(argv[cnt], "-spr_maxtrav") == 0 ||
                strcmp(argv[cnt], "-spr_rad") == 0) {
                cnt++;
                if (cnt >= argc)
                    throw "Use " + string(argv[cnt]) + " <maximal SPR radius>";
                params.spr_maxtrav = convert_int(argv[cnt]);
                params.sprDist =
                    params.spr_maxtrav; // Diep: hopefully this speed the
                                        // pllMakeParsimonyTreeFast...
                continue;
            }
            ...
            if (strcmp(argv[cnt], "-cand_cutoff") == 0) {
                cnt++;
                if (cnt >= argc)
                    throw "Use -cand_cutoff <integer #s from 0 to 100 for "
                          "selecting top "
                          "#s percentile as bootstrap candidates>";
            ...
            if (strcmp(argv[cnt], "-test_mode") == 0) {
                //            	if(argc != 7)
                //            		throw "These options are to compute
                //            weighted parsimony score.\nUse -s [alignment_file]
                //            -test_mode [tree_file] -cost fitch/[cost_file]";
                // if(argc != 5)
            ...
            if (strcmp(argv[cnt], "-distinct_iter_top_boot") == 0) {
                cnt++;
                if (cnt >= argc)
                    throw "Use -distinct_iter_top_boot <# of boot trees "
                          "(before "
                          "refinement step) for each boot aln>";
            ...
            if (strcmp(argv[cnt], "-snni") == 0) {
                ...
                // Minh: why do you turn this on? it doubles curPerStrength at
                // some point params.adaptPert = true;
                continue;
            }
            ...
            if (strcmp(argv[cnt], "-pb") ==
                0) { // Enable parsimony branch length estimation
                params.parbran = true;
                continue;
            }
            ...
            if (strcmp(argv[cnt], "-sprdist") == 0) {
                cnt++;
                if (cnt >= argc)
                    throw "Use -sprdist <SPR distance used in parsimony "
                          "search>";
    ...
    if (!params.user_file && !params.aln_file && !params.ngs_file &&
        !params.ngs_mapped_reads && !params.partition_file)
#ifdef IQ_TREE
        //        usage_iqtree(argv, false);
        //		usage_mpboot(argv, false);
        usage(argv, false);
    ...
    } else if (params.optimize_boot_trees == true &&
               params.save_trees_off == true) {
        params.stop_condition = SC_UNSUCCESS_ITERATION;
    }
}
...
void usage(char *argv[], bool full_command) {
    ...
    cout << "Minimal command-line examples (replace 'mpboot ...' with actual "
            "path to executable):"
         << endl
         << endl;

    cout << "1. Reconstruct maximum parsimony tree from a sequence alignment"
         << endl
         << "   (example.phy):" << endl
         << "     mpboot -s example.phy" << endl
         << endl;

    cout << "2. Reconstruct MP tree and assess branch supports with the MPBoot "
            "method"
         << endl
    ...
    cout << "Have a look at the tutorial and manual for more information:"
         << endl
         << "     http://www.cibiv.at/software/mpboot" << endl
         << endl;

    //    cout << "Usage: " << argv[0] << " [OPTIONS] <file_name>
    //    [<output_file>]"
    //    << endl; cout << "GENERAL OPTIONS:" << endl; cout << "  -hh Print this
    //    help dialog" << endl; cout << "  -h                Print help options
    //    for phylogenetic inference" << endl; cout << "  <file_name>       User
    //    tree in NEWICK format or split network in NEXUS format" << endl; cout
    //    << "  <output_file>     Output file to store results, default is
    //    '<file_name>.pda'" << endl; cout << "  -k <num_taxa>     Find optimal
    //    set of size <num_taxa>" << endl; cout << "  -k <min>:<max>    Find
    //    optimal sets of size from <min> to <max>" << endl; cout << "  -k
    //    <min>:<max>:<step>" << endl; cout << "                    Find optimal
    //    sets of size min, min+step, min+2*step,..." << endl; cout << "  -o
    //    <taxon>        Root name to compute rooted PD (default: unrooted)" <<
    //    endl; cout << "  -if <file>        File containing taxa to be included
    //    into optimal sets" << endl; cout << "  -e <file>         File
    //    containing branch/split scale and taxa weights" << endl; cout << "
    //    -all Identify all multiple optimal sets" << endl; cout << "  -lim
    //    <max_limit>  The maximum number of optimal sets for each k if -a is
    //    specified" << endl; cout << "  -min              Compute minimal sets
    //    (default: maximal)" << endl; cout << "  -1out             Print taxa
    //    sets and scores to separate files" << endl; cout << "  -oldout Print
    //    output compatible with version 0.3" << endl; cout << "  -v Verbose
    //    mode" << endl; cout << endl; cout << "OPTIONS FOR PHYLOGENETIC
    //    DIVERSITY (PD):"
    //    << endl; cout << "  -root             Make the tree ROOTED, default is
    //    unrooted" << endl; cout << "    NOTE: this option and -o <taxon>
    //    cannot be both specified" << endl; cout << "  -g                Run
    //    greedy algorithm only (default: auto)" << endl; cout << "  -pr Run
    //    pruning algorithm only (default: auto)" << endl; cout << endl;
    //    /*
    //    cout << "OPTIONS FOR SPLIT DIVERSITY:" << endl;
    //    cout << "  -exhaust          Force to use exhaustive search" << endl;
    //    cout << "    NOTE: by default, the program applies dynamic programming
    //    algorithm" << endl; cout << "          on circular networks and
    //    exhaustive search on general networks" << endl; cout << endl;*/ cout
    //    << "OPTIONS FOR BUDGET CONSTRAINTS:" << endl; cout << "  -u <file>
    //    File containing total budget and taxa preservation costs" << endl;
    //    cout << " -b <budget>       Total budget to conserve taxa" << endl;
    //    cout << "  -b <min>:<max>    Find all sets with budget from <min> to
    //    <max>" << endl; cout << "  -b <min>:<max>:<step>" << endl; cout << "
    //    Find optimal sets with budget min, min+step, min+2*step,..." << endl;
    //    cout << endl; cout << "OPTIONS FOR AREA ANALYSIS:" << endl; cout << "
    //    -ts <taxa_file>   Compute/maximize PD/SD of areas (combine with -k to
    //    maximize)" << endl; cout << "  -excl Compute exclusive PD/SD" << endl;
    //    cout << "  -endem            Compute endemic PD/SD" << endl; cout << "
    //    -compl <areas>    Compute complementary PD/SD given the listed
    //    <areas>" << endl; cout << endl;
    //
    //    cout << "OPTIONS FOR VIABILITY CONSTRAINTS:" << endl;
    //    cout << "  -eco <food_web>   File containing food web matrix" << endl;
    //    cout << "  -k% <n>           Find optimal set of size relative the
    //    total number of taxa" << endl; cout << "  -diet <min_diet>  Minimum
    //    diet portion (%) to be preserved for each predator" << endl; cout <<
    //    endl;
    //    //if (!full_command) exit(0);
    //
    //    cout << "MISCELLANEOUS:" << endl;
    //    cout << "  -dd <sample_size> Compute PD distribution of random sets of
    //    size k" << endl;
    //    /*
    //    cout << "  -gbo <sitelh_file> Compute and output the alignment of
    //    (normalized)" << endl; cout << "                    expected
    //    frequencies given in site_ll_file" << endl;
    //	*/
    //
    //    //	cout << "  -rep <times>        Repeat algorithm a number of
    //    times." << endl;
    //    //	cout << "  -noout              Print no output file." << endl;
    //    cout << endl;
    //    //cout << "HIDDEN OPTIONS: see the source code file
    //    pda.cpp::parseArg()"
    //    << endl;
...
void usage_iqtree(char *argv[], bool full_command) {
    printCopyright(cout);
    cout << "Usage: " << argv[0] << " -s <alignment> [OPTIONS] [<treefile>] "
         << endl
         << endl;
    cout
        << "GENERAL OPTIONS:" << endl
        << "  -?                   Printing this help dialog" << endl
        << "  -s <alignment>       Input alignment (REQUIRED) in "
           "PHYLIP/FASTA/NEXUS format"
        << endl
        << "  -st <data_type>      BIN, DNA, AA, CODON, or MORPH (default: "
           "auto-detect)"
        << endl
        << "  -sp <partition_file> Partition model specification in NEXUS "
           "format."
        << endl
        << "                       For single model use the -m option (see "
           "below)"
        << endl
        << "  -z <trees_file>      Compute log-likelihoods for all trees in "
           "the "
           "given file"
        << endl
        << "  <treefile>           Initial tree for tree reconstruction "
           "(default: MP)"
        << endl
        << "  -o <outgroup_taxon>  Outgroup taxon name for writing .treefile"
        << endl
        << "  -pre <PREFIX>        Using <PREFIX> for output files (default: "
           "alignment name)"
        << endl
#ifdef _OPENMP
        << "  -omp <#cpu_cores>    Number of cores/threads to use (default: "
           "all "
           "cores)"
        << endl
#endif
        << "  -seed <number>       Random seed number, normally used for "
           "debugging purpose"
        << endl
        << "  -v, -vv, -vvv        Verbose mode, printing more messages to "
           "screen"
        << endl
        << endl
        << "NEW STOCHASTIC TREE SEARCH ALGORITHM:" << endl
        << "  -pll                 Use phylogenetic likelihood library (PLL) "
           "(default: off)"
        << endl
        << "  -numpars <number>    Number of initial parsimony trees (default: "
           "100)"
        << endl
        ...
        << endl
        << "  -pers <perturbation> Perturbation strength for randomized NNI "
           "(default: 0.5)"
        << endl
        << "  -numstop <number>    Number of unsuccessful iterations to stop "
           "(default: 100)"
        << endl
        << "  -n <#iterations>     Fix number of iterations to <#iterations> "
           "(default: auto)"
        << endl
        << "  -iqp                 Use the IQP tree perturbation (default: "
           "randomized NNI)"
        << endl
        << "  -iqpnni              Switch back to the old IQPNNI tree search "
           "algorithm"
        << endl
        << endl
        ...
        << endl
        //            << "  -n <#iterations>     Minimum number of iterations
        //            (default: 100)" << endl
        << "  -nm <#iterations>    Maximum number of iterations (default: 1000)"
        << endl
        << "  -nstep <#iterations> #Iterations for UFBoot stopping rule "
           "(default: 100)"
        << endl
        << "  -bcor <min_corr>     Minimum correlation coefficient (default: "
           "0.99)"
        << endl
        ...
        << "  -t <threshold>       Minimum bootstrap support [0...1) for "
           "consensus tree"
        << endl
        << endl
        ...
        << "  -m <model_name>" << endl
        << "                  DNA: HKY (default), JC, F81, K2P, K3P, K81uf, "
           "TN/TrN, TNef,"
        << endl
        << "                       TIM, TIMef, TVM, TVMef, SYM, GTR, or "
           "6-digit "
           "model"
        << endl
        << "                       specification (e.g., 010010 = HKY)" << endl
        << "              Protein: WAG (default), Poisson, cpREV, mtREV, "
           "Dayhoff, mtMAM,"
        << endl
        << "                       JTT, LG, mtART, mtZOA, VT, rtREV, DCMut, "
           "PMB, "
           "HIVb,"
        << endl
        ...
        << "      Model selection: TEST or TESTONLY to auto-select the "
           "best-fit "
           "model."
        << endl
        << "                       TESTONLY will stop the run after model "
           "selection"
        << endl
        << "            Otherwise: Name of file containing user-model "
           "parameters"
        << endl
        << "                       (rate parameters and state frequencies)"
        << endl
        << "  -m <model_name>+F or +FO or +FU or +FQ (default: auto)" << endl
        << "                       counted, optimized, user-defined, equal "
           "state "
           "frequency"
        << endl
        << "  -m <model_name>+F1x4 or +F3x4 or +F3x4C" << endl
        << "                       Codon frequencies" << endl
        << "  -m <model_name>+ASC  Ascertainment bias correction for "
           "morphological/SNP data"
        << endl
        << endl
        << "RATE HETEROGENEITY:" << endl
        << "  -m <model_name>+I or +G[n] or +I+G[n]" << endl
        << "                       Invar, Gamma, or Invar+Gamma rates. 'n' is "
           "number of"
        << endl
        << "                       categories for Gamma rates (default: n=4)"
        << endl
        << "  -a <Gamma_shape>     Gamma shape parameter for site rates "
           "(default: estimate)"
        << endl
        << "  -gmedian             Computing mean for Gamma rate category "
           "(default: mean)"
        << endl
        << "  -i <p_invar>         Proportion of invariable sites (default: "
           "estimate)"
        << endl
        << "  -mh                  Computing site-specific rates to .mhrate "
           "file "
           "using"
        << endl
        ...
        << "  -m WHTEST            Testing model (GTR+G) homogeneity "
           "assumption "
           "using"
        << endl
        << "                       Weiss & von Haeseler (2003) method" << endl
        << "  -ns <#simulations>   #Simulations to obtain null-distribution "
           "(default: 1000)"
        << endl
        //            << endl << "TREE INFERENCE:" << endl
        //            << "  -p <probability>     IQP: Probability of deleting
        //            leaves (default: auto)" << endl
        //            << "  -k <#representative> IQP: Size of representative
        //            leaf set (default: 4)" << endl
        //            << "  -n <#iterations>     Number of iterations  (default:
        //            auto)" << endl
        //            << "  -sr <#iterations>    Stopping rule with max.
        //            #iterations (default: off)" << endl
        //            << "  -sc <confidence>     Confidence value for stopping
        //            rule (default: 0.95)" << endl
        //            << "  -spc <level>         Confidence level for NNI
        //            adaptive search (default 0.95)" << endl
        //            << "  -sp_iter <number>    #iterations before NNI adaptive
        //            heuristic is started" << endl
        //            << "  -lmd <lambda>        lambda parameter for the PhyML
        //            search (default 0.75)" << endl
        //            << "  -nosse               Disable SSE instructions" <<
        //            endl
        //            << "  -wt                  Writing all intermediate trees
        //            into .treels file" << endl
        //            << "  -d <file>            Reading genetic distances from
        //            file (default: JC)" << endl
        //            << "  -fixbr               Fix branch lengths of
        //            <treefile>"
        //            << endl
        //            << "  -seed <number>       Random seed number, normally
        //            used for debugging purpose" << endl
        //            << "  -v, -vv, -vvv        Verbose mode, printing more
        //            messages to screen" << endl
        << endl
        << "CONSENSUS RECONSTRUCTION:" << endl
        << "  <tree_file>          Set of input trees for consensus "
           "reconstruction"
        << endl
        << "  -t <threshold>       Min split support in range [0,1]. 0.5 for "
           "majority-rule"
        << endl
        << "                       consensus (default: 0, i.e. extended "
           "consensus)"
        << endl
        << "  -bi <burnin>         Discarding <burnin> trees at beginning of "
           "<treefile>"
        << endl
        ...
        << endl
        << "  -sup <target_tree>   Assigning support values for <target_tree> "
           "to "
           ".suptree"
        << endl
        << endl
        << "ROBINSON-FOULDS DISTANCE:" << endl
        << "  -rf_all              Computing all-to-all RF distances of trees "
           "in "
           "<treefile>"
        << endl
        << "  -rf <treefile2>      Computing all RF distances between two sets "
           "of trees"
        << endl
        << "                       stored in <treefile> and <treefile2>" << endl
        << "  -rf_adj              Computing RF distances of adjacent trees in "
           "<treefile>"
        << endl
        << endl
        << "TREE TOPOLOGY TEST:" << endl
        << "  -zb <#replicates>    BP,KH,SH,ELW tests with RELL for trees "
           "passed "
           "via -z"
        << endl
        << "  -zw                  Also performing weighted-KH and weighted-SH "
           "tests"
        << endl
        << endl;

    cout << "GENERATING RANDOM TREES:" << endl;
    cout << "  -r <num_taxa>        Create a random tree under Yule-Harding "
            "model."
         << endl;
    cout << "  -ru <num_taxa>       Create a random tree under Uniform model."
         << endl;
    cout << "  -rcat <num_taxa>     Create a random caterpillar tree." << endl;
    cout << "  -rbal <num_taxa>     Create a random balanced tree." << endl;
    cout << "  -rcsg <num_taxa>     Create a random circular split network."
         << endl;
    cout << "  -rlen <min_len> <mean_len> <max_len>  " << endl;
    cout
        << "                       min, mean, and max branch lengths of random "
           "trees."
        << endl;
    ...
    cout
        << endl
        << "MISCELLANEOUS:" << endl
        << "  -wt                  Write locally optimal trees into .treels "
           "file"
        << endl
        << "  -fixbr               Fix branch lengths of <treefile>." << endl
        << "                       Used with -n 0 to compute log-likelihood of "
           "<treefile>"
        << endl
        << "  -wsl                 Writing site log-likelihoods to .sitelh file"
        << endl
        << "  -wslg                Writing site log-likelihoods per Gamma "
           "category"
        << endl;
    //            << "  -d <file>            Reading genetic distances from file
    //            (default: JC)" << endl
    //			<< "  -d <outfile>         Calculate the distance matrix
    // inferred from tree" << endl
    //			<< "  -stats <outfile>     Output some statistics about
    // branch lengths" << endl
    //			<< "  -comp <treefile>     Compare tree with each in the
    // input trees" << endl;
...
void usage_mpboot(char *argv[], bool full_command) {
    ...
    cout
        ...
        << "  -mulhits                  Store multiple equally parsimonious "
           "trees per bootstrap replicate"
        << endl
        << "  -ratchet_iter <number>    Number of non-ratchet iterations "
           "before "
           "each ratchet iteration (default: 1)"
        << endl
        << "  -ratchet_wgt <number>     Weight to add to each site selected "
           "for "
           "perturbation during ratchet (default: 1)"
        << endl
        << "  -ratchet_percent <number> Percentage of informative sites "
           "selected "
           "for perturbation during ratchet (default: 50)"
        << endl
        << "  -ratchet_off              Turn of ratchet, i.e. Only use tree "
           "perturbation"
        << endl
        << "  -spr_rad <number>         Maximum radius of SPR (default: 3)"
        << endl
        << "  -cand_cutoff <#s>         Use top #s percentile as cutoff for "
           "selecting bootstrap candidates (default: 10)"
        << endl
        << "  -opt_btree_off            Turn off refinement step on the final "
           "bootstrap tree set"
        << endl
        << "  -nni_pars                 Hill-climb by NNI instead of SPR"
        << endl
        << "  -cost <file>              Read <file> for the matrix of "
           "transition "
           "cost between character states"
        << endl
        << "                            Replace <file> by letter e for uniform "
           "cost."
        << endl
    ...
    cout << "  -wspars              When using together with parsimony tree "
            "inference, print site parsimony scores of the best tree found."
         << endl;
    cout << "  -wspars-user-tree <treefile> Print site parsimony scores of the "
            "user tree in <treefile>"
         << endl
         << endl;
...
int init_random(int seed) /* RAND4 */ {
...
#else /* PARALLEL */
    {
        int n;
        if (PP_IamMaster) {
            cout << "(Using RAND4 Random Number Generator with leapfrog method)"
                 << endl;
        }
        for (n = 0; n < PP_Myid; n++)
            (void)randomunitintervall();
        if (verbose_mode >= VB_MED) {
            cout << "(" << PP_Myid << ") !!! random seed set to " << seed
                 << ", " << n << " drawn !!!" << endl;
        }
    }
#endif
...
#else  /* PARALLEL */
    if (PP_IamMaster) {
        cout << "(Using SPRNG - Scalable Parallel Random Number Generator)"
             << endl;
    }
    /* MPI_Bcast(&seed, 1, MPI_UNSIGNED, PP_MyMaster, MPI_COMM_WORLD); */
    randstream = init_sprng(PP_Myid, PP_NumProcs, seed,
                            SPRNG_DEFAULT); /*initialize stream*/
    if (verbose_mode >= VB_MED) {
        cout << "(" << PP_Myid << ") !!! random seed set to " << seed << " !!!"
             << endl;
        print_sprng(randstream);
...
int finish_random() { return free_sprng(randstream); }
...
useful for absolute z values < 6.  For z values >= to 6.0, Normalz() returns
0.0.
 */

double Normalz(double z) /*VAR returns cumulative probability from -oo to z VAR
                            normal z value */
{
    double y, x, w;
...
Adapted from: 	Hill, I. D. and Pike, M. C.  Algorithm 299.Collected Algorithms
for the CACM 1967 p. 243
Updated for rounding errors based on remark inACM TOMS June 1985, page 185.
Found in Perlman.lib*/

double computePValueChiSquare(
    double x,
    int df) /* x: obtained chi-square value,  df: degrees of freedom */ {
    double a, y, s;
    double e, c, z;
