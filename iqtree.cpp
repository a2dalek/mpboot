...
if (params.gbo_replicates)
  params.max_iterations = max(params.max_iterations, max(params.min_iterations,
1000));
*/

  if (params.gbo_replicates & !params.maximum_parsimony) {
    params.max_iterations =
        max(params.max_iterations, max(params.min_iterations, 1000));
  }
  ...
  */
  ...
  if (params.online_bootstrap && params.gbo_replicates > 0) {
    cout << "Generating " << params.gbo_replicates
         << " samples for bootstrap approximation..." << endl;
    ...
    if (params.maximum_parsimony) {
      ...
      // (size_t)(params.gbo_replicates)); 			memset(mem, 0,
      // nunit *
      //(size_t)(params.gbo_replicates) * sizeof(BootValTypePars));
...
#endif
      BootValType *mem =
          aligned_alloc<BootValType>(nunit * (size_t)(params.gbo_replicates));
      memset(mem, 0,
             nunit * (size_t)(params.gbo_replicates) * sizeof(BootValType));
      for (i = 0; i < params.gbo_replicates; i++)
    ...
    if (params.maximum_parsimony)
      boot_logl.resize(
          ...
          -LONG_MAX); // Diep: Leaving this out might affect REPS computation
    else
...
void IQTree::initTopologyByPLLRandomAdition(Params &params){
  ...
  pllQueue *partitionInfo =
      pllPartitionParseString(pllPartitionFileHandle.str().c_str());
...
BootValTypePars *IQTree::getPatternPars() { return _pattern_pars; }
...
void IQTree::createPLLPartition(Params &params,
  ...
  if (isSuperTree()) {
    ...
    if (siqtree->size() > PLL_NUM_BRANCHES)
      outError("Number of partitions exceeds PLL limit, please increase "
               "PLL_NUM_BRANCHES constant in pll.h");
    int i = 0;
    int startPos = 1;
    for (PhyloSuperTree::iterator it = siqtree->begin(); it != siqtree->end();
         it++) {
      i++;
      ...
      if ((*it)->aln->seq_type == SEQ_DNA) {
        ...
        if (siqtree->part_info[i - 1].model_name != "" &&
            siqtree->part_info[i - 1].model_name.substr(0, 4) != "TEST")
          pllPartitionFileHandle << siqtree->part_info[i - 1].model_name.substr(
              0, siqtree->part_info[i - 1].model_name.find_first_of("+{"));
        else
    ...
    if (aln->seq_type == SEQ_DNA) {
      ...
      if (params.pll && params.model_name != "" &&
        ...
        model =
            params.model_name.substr(0, params.model_name.find_first_of("+{"));
      } else {
        model = "WAG";
      }
    ...
    if (!getModelFactory()) {
      if (isSuperTree()) {
        if (params.partition_type) {
          setModelFactory(
              new PartitionModelPlen(params, (PhyloSuperTreePlen *)this));
        } else
...
double IQTree::getProbDelete() { return (double)k_delete / leafNum; }
...
// void IQTree::setIQPIterations(STOP_CONDITION stop_condition, double
// stop_confidence, int min_iterations,
//         int max_iterations) {
...
RepresentLeafSet *
IQTree::findRepresentLeaves(vector<RepresentLeafSet *> &leaves_vec, int nei_id,
                            PhyloNode *dad) {
...
/*
 void IQPTree::clearRepresentLeaves(vector<RepresentLeafSet*> &leaves_vec, Node
 *node, Node *dad) { int nei_id; for (nei_id = 0; nei_id <
 node->neighbors.size(); nei_id++) if (node->neighbors[nei_id]->node == dad)
 break; assert(nei_id < node->neighbors.size()); int set_id = node->id * 3 +
 nei_id; if (leaves_vec[set_id]) { for (RepresentLeafSet::iterator rlit =
 leaves_vec[set_id]->begin(); rlit != leaves_vec[set_id]->end(); rlit++) delete
 (*rlit); delete leaves_vec[set_id]; leaves_vec[set_id] = NULL;
 }
...
void IQTree::deleteNonCherryLeaves(PhyloNodeVector &del_leaves) {
  ...
  for (vector<unsigned int>::iterator it = indices_noncherry.begin();
  ...
  if (i < num_delete) {
    ...
    startValue = 0;
    for (vector<unsigned int>::iterator it = indices_cherry.begin();
    ...
    for (RepresentLeafSet::iterator i1 = leaves[1]->begin();
      ...
      for (RepresentLeafSet::iterator i2 = leaves[2]->begin();
        ...
        if (iqp_assess_quartet == IQP_DISTANCE)
          best_id =
              assessQuartet((*i0)->leaf, (*i1)->leaf, (*i2)->leaf, del_leaf);
        else
...
void IQTree::reinsertLeaves(PhyloNodeVector &del_leaves) {
  ...
  for (it_leaf = del_leaves.begin(); it_leaf != del_leaves.end(); it_leaf++) {
    ...
    if (verbose_mode >= VB_DEBUG)
      drawTree(cout,
               WT_BR_SCALE | WT_INT_NODE | WT_TAXON_ID | WT_NEWLINE | WT_BR_ID);
    ...
    if (best_nodes.size() > 1 && verbose_mode >= VB_DEBUG)
      cout << best_nodes.size()
           << " branches show the same best bonus, branch nr. " << node_id
           << " is chosen" << endl;
    ...
    for (vector<RepresentLeafSet *>::iterator rit = leaves_vec.begin();
      ...
      if ((*rit)) {
        ...
        for (RepresentLeafSet::iterator rlit = tit->begin(); rlit != tit->end();
             rlit++)
          delete (*rlit);
        delete (*rit);
      }
  }
  initializeTree(); // BQM: re-index nodes and branches s.t. ping-pong neighbors
                    // have the same ID
...
void IQTree::doRandomNNIs(int numNNI) {
  ...
  for (int i = 0; i < numNNI; i++) {
    ...
    if (usedNodes.find(nodeList1[index]->id) == usedNodes.end() &&
      ...
      usedNodes.insert(
          map<int, Node *>::value_type(nodeList1[index]->id, nodeList1[index]));
      usedNodes.insert(
          map<int, Node *>::value_type(nodeList2[index]->id, nodeList2[index]));
    } else {
      usedNodes.clear();
      nodeList1.clear();
      nodeList2.clear();
...
double IQTree::inputTree2PLL(string treestring, bool computeLH) {
  ...
  if (computeLH) {
    pllEvaluateLikelihood(pllInst, pllPartitions, pllInst->start, PLL_TRUE,
                          PLL_FALSE);
...
double *IQTree::getModelRatesFromPLL() {
  ...
  int numberOfRates = (pllPartitions->partitionData[0]->states *
                           pllPartitions->partitionData[0]->states -
                       pllPartitions->partitionData[0]->states) /
                      2;
...
void IQTree::inputModelPLL2IQTree() {
  ...
  dynamic_cast<RateGamma *>(getRate())->setGammaShape(
      pllPartitions->partitionData[0]->alpha);
  if (aln->num_states == 4) {
    ((ModelGTR *)getModel())
        ->setRateMatrix(pllPartitions->partitionData[0]->substRates);
    getModel()->decomposeRateMatrix();
  }
  ((ModelGTR *)getModel())
      ->setStateFrequency(
          pllPartitions->partitionData[0]->empiricalFrequencies);
}
...
void IQTree::inputModelIQTree2PLL() {
  ...
  if (aln->num_states == 4) {
    ...
    for (partNr = 0; partNr < pllPartitions->numberOfPartitions; partNr++) {
      ...
      pllSetFixedBaseFrequencies(state_freqs, 4, partNr, pllPartitions,
                                 pllInst);
      pllSetFixedSubstitutionMatrix(rate_param, 6, partNr, pllPartitions,
                                    pllInst);
    }
    delete[] rate_param;
    delete[] state_freqs;
    ...
    for (partNr = 0; partNr < pllPartitions->numberOfPartitions; partNr++) {
      pllSetFixedAlpha(alpha, partNr, pllPartitions, pllInst);
      pllSetFixedBaseFrequencies(state_freqs, 20, partNr, pllPartitions,
                                 pllInst);
    }
    delete[] state_freqs;
  } else {
    if (params->pll) {
      outError("Phylogenetic likelihood library current does not support data "
               "type other than DNA or Protein");
    }
  }
}
...
void IQTree::pllBuildIQTreePatternIndex() {
  ...
  for (int i = 0; i < pllAlignment->sequenceCount; i++) {
    ...
    for (int model = 0; model < pllPartitions->numberOfPartitions; model++) {
      memcpy(&pll_aln[i][pos],
             &pllAlignment
                  ->sequenceData[i + 1]
                                [pllPartitions->partitionData[model]->lower],
  ...
  for (int i = 0; i < pllAlignment->sequenceLength; i++) {
    bool check_base_subst = false; // Diep: (Sep 24, 2014) Temp solution to the
                                   // irregular on/off base substitute of PLL
    ...
    for (int k = 0; k < aln->size(); k++) {
      ...
      if (memcmp(pll_site, site, pllAlignment->sequenceCount) == 0) {
        ...
        //                if(k == 104){
        //                	for(int p = 0; p < pllAlignment->sequenceCount;
        //                p++){
        //                        site[p] =
        //                        aln->convertStateBack(aln->at(k)[p]); cout <<
        //                        (char)site[p];
        //                	}
        //                	cout << "$$$$$$$$$$$$$$$$$$" << endl;
        //                }
...
void IQTree::pllBaseSubstitute(char *seq, int dataType) {
  ...
  meaningDNA[(int)'N'] = meaningDNA[(int)'n'] = meaningDNA[(int)'O'] =
      meaningDNA[(int)'o'] = meaningDNA[(int)'X'] = meaningDNA[(int)'x'] =
          meaningDNA[(int)'-'] = meaningDNA[(int)'?'] = 15;
  ...
  meaningAA[(int)'X'] = meaningAA[(int)'x'] = meaningAA[(int)'?'] =
      meaningAA[(int)'*'] = meaningAA[(int)'-'] = 22;
...
string IQTree::optimizeModelParameters(bool printInfo) {
  ...
  if (params->pll) {
    ...
    cout << "Estimate model parameters (epsilon = " << params->modeps << ")"
         << endl;
    double modOptScore = getModelFactory()->optimizeParameters(
        params->fixed_branch_length, printInfo, params->modeps);
    if (isSuperTree()) {
    ...
    if (modOptScore < curScore - 1.0) {
      ...
      //				((GTRModel*)
      // getModel())->setRateMatrix(rate_param_bk);
      //			}
...
void IQTree::printBestScores(int numBestScore) {
  vector<double> bestScores =
      candidateTrees.getBestScores(candidateTrees.popSize);
  for (vector<double>::iterator it = bestScores.begin(); it != bestScores.end();
       it++)
  ...
  for (; !stop_rule.meetStopCondition(curIt, cur_correlation); curIt++) {
    ...
    if (params->cutoff_percent > 100) {
      ...
      if (params->avoid_duplicated_trees && max_candidate_trees > 0 &&
        ...
        int predicted_iteration =
            ((curIt + params->step_iterations - 1) / params->step_iterations) *
            params->step_iterations;
        int num_entries =
            floor(max_candidate_trees * ((double)curIt / predicted_iteration));
          ...
          if (curIt % 10 == 0) {
            ...
            if (params->store_candidate_trees)
              cout << " duplicates= " << duplication_counter << " ("
                   << (int)round(100 * ((double)duplication_counter /
                                        treels_logl.size()))
                   << "%)" << endl;
            else
              cout << endl;
      ...
      if (params->cutoff_from_btrees) {
        ...
        logl_cutoff = *min_element(boot_tree_orig_logl.begin(),
                                   boot_tree_orig_logl.end());
      } else {
        ...
        if (params->avoid_duplicated_trees && treels_logl.size() > 1000) {
          ...
          if (params->minimize_iter1_candidates) {
            if (curIt == 2) {
              int iter1_num_best =
                  min(int(aln->getNSeq()),
                      int(logl.size() * params->cutoff_percent / 100));
    ...
    if (params->ratchet_iter >= 0) {
      if (params->ratchet_iter == ratchet_iter_count) {
        ...
        perturb_alignment->createPerturbAlignment(aln, params->ratchet_percent,
                                                  params->ratchet_wgt,
    ...
    if (!on_ratchet_hclimb1) {
      if (iqp_assess_quartet == IQP_BOOTSTRAP) {
        ...
        if (params->snni) {
          ...
          // cout << "candidateTrees.size() = " << candidateTrees.size() <<
          // endl;
          //					string candidateTree =
          // candidateTrees.getRandCandVecTree(); // Diep: to pick from
          // vector-stored candidates
          ...
          if (params->iqp) {
            ...
            doRandomNNIs(numNNI); // Diep: This doesn't work well with sorted
                                  // parsimony. Why?
          }
        } else {
          doIQP();
        ...
        if (params->maximum_parsimony && params->spr_parsimony &&
            (params->snni ||
             params->pll)) { // SPR for mpars
                             ...
                             //*perturbTree
                             //=
          // pllNewickParseString(perturb_tree_string.c_str());
          //					assert(perturbTree != NULL);
          //					pllTreeInitTopologyNewick(pllInst,
          // perturbTree, PLL_FALSE);
          initializeAllPartialPars();
          clearAllPartialLH();
          curScore = perturbScore = -computeParsimony();
          //                    pllNewickParseDestroy(&perturbTree);
        } else if (params->pll) {
          pllNewickTree *perturbTree =
              pllNewickParseString(perturb_tree_string.c_str());
          assert(perturbTree != NULL);
          pllTreeInitTopologyNewick(pllInst, perturbTree, PLL_FALSE);
          pllEvaluateLikelihood(pllInst, pllPartitions, pllInst->start,
                                PLL_TRUE, PLL_FALSE);
          if (params->numSmoothTree >= 1) {
            pllOptimizeBranchLengths(pllInst, pllPartitions,
                                     params->numSmoothTree);
          }
          pllNewickParseDestroy(&perturbTree);
          curScore = pllInst->likelihood;
          ...
          curScore = optimizeAllBranches(params->numSmoothTree, TOL_LIKELIHOOD,
                                         PLL_NEWZPERCYCLE);
          perturbScore = curScore;
        }
      }
    ...
    double realtime_remaining =
        stop_rule.getRemainingTime(curIt, cur_correlation);
    ...
    if (curIt % 10 == 0 || verbose_mode >= VB_MED) {
      ...
      cout << ((iqp_assess_quartet == IQP_BOOTSTRAP) ? "Bootstrap "
                                                     : "Iteration ")
           << curIt << (params->maximum_parsimony ? " / Score: " : " / LogL: ");
      if (verbose_mode >= VB_MED)
        cout << perturbScore << " -> ";
...
//        if (curScore > bestScore) { // Minh&Tung for ML
           if (curScore > bestScore || (curScore == bestScore &&
params->maximum_parsimony)) { // Diep added condition for MP stringstream
cur_tree_topo_ss; setRootNode(params->root); printTree(cur_tree_topo_ss,
WT_TAXON_ID | WT_SORT_TAXA);

                   bool is_new_tree = false;
                   if(params->maximum_parsimony)
                           is_new_tree =
!candidateTrees.treeTopologyExist(cur_tree_topo_ss.str()); else is_new_tree =
(cur_tree_topo_ss.str() != best_tree_topo);

       if (is_new_tree) {
           best_tree_topo = cur_tree_topo_ss.str();
           // Diep: fix Minh's old if which wrongly set imd_tree =
best_tree_topo for mpars if (!params->maximum_parsimony) imd_tree =
optimizeModelParameters(); stop_rule.addImprovedIteration(curIt); if(curScore ==
bestScore && params->maximum_parsimony) cout << "NOTE: A new MP tree with same
score as the best." << endl; cout << "BETTER TREE FOUND at iteration " << curIt
<< ": " << (-curScore); cout << " / CPU time: " << (int) round(getCPUTime() -
params->startCPUTime) << "s" << endl << endl; if (curScore > bestScore) {
               searchinfo.curPerStrength = params->initPerStrength;
           }
       } else {
           if(!params->maximum_parsimony)
    ...
    if (curScore > bestScore) {
      ...
      if (cur_tree_topo_ss.str() != best_tree_topo) {
        ...
        cout << " / CPU time: "
             << (int)round(getCPUTime() - params->startCPUTime) << "s" << endl
             << endl;
        if (curScore > bestScore) {
          searchinfo.curPerStrength = params->initPerStrength;
        }
    ...
    if ((!params->maximum_parsimony) && (params->pll) &&
        (params->online_bootstrap) && (params->gbo_replicates > 0))
     ...
     *---------------------------------------*/
    if ((curIt) % (params->step_iterations / 2) == 0 &&
      ...
      if (params->max_candidate_trees == 0)
        max_candidate_trees = treels_logl.size() *
                              (curIt + (params->step_iterations / 2)) / curIt;
      string cutoff_name =
          params->maximum_parsimony ? "candidate-score-cutoff" : "logl-cutoff";
      cout << "NOTE: " << treels_logl.size()
           << " bootstrap candidate trees evaluated (" << cutoff_name << ": "
      ...
      if (curIt % params->step_iterations == 0) {
        ...
        cout.precision(3);
        cout << "NOTE: Bootstrap correlation coefficient of split occurrence "
                "frequencies: "
        ...
        if (!stop_rule.meetStopCondition(curIt, cur_correlation)) {
          if (params->max_candidate_trees == 0) {
            max_candidate_trees =
                treels_logl.size() * (curIt + params->step_iterations) / curIt;
          }
          //	                cout << "INFO: UFBoot does not converge,
          // continue " << params->step_iterations << " more iterations" <<
          // endl;
        }
      }
  ...
  if (params->gbo_replicates &&
      ((curIt - 1) % (params->step_iterations / 2) != 0)) {
    cout << "NOTE: At the end, " << treels_logl.size()
...
string IQTree::doNNISearch(int &nniCount, int &nniSteps) {
  ...
  if (params->maximum_parsimony && params->spr_parsimony &&
    ...
    if (on_opt_btree) {
      ...
      // Diep: sorting the boot aln is needed for branch and bound weighted
      // search
      PatternComp pcomp;
      ...
      initializePLL(
          *params); // because the set of patterns might be a subset of the orig
    ...
    if (on_ratchet_hclimb1 && params->hclimb1_nni) {
      ...
      // deallocation will occur once at the end of runTreeReconstruction() if
      // not running ratchet oct 23: in non-ratchet iteration, free is not
      // triggered
...
double IQTree::optimizeNNI(int &nni_count, int &nni_steps) {
  ...
  for (nni_steps = 1; nni_steps <= MAXSTEPS; nni_steps++) {
    ...
    if (!rollBack) { // tree get improved and was not rollbacked
      ...
      if (verbose_mode >= VB_DEBUG) {
        ...
        for (int i = 0; i < plusNNIs.size(); i++) {
          cout << "Logl of positive NNI " << i << " : " << plusNNIs[i].newloglh
               << endl;
        }
      }
      ...
      if (verbose_mode >= VB_DEBUG) {
        for (int i = 0; i < nonConfNNIs.size(); i++) {
          cout << "Log-likelihood of non-conflicting NNI " << i << " : "
               << nonConfNNIs[i].newloglh << endl;
        }
      }
    }
    ...
    if (verbose_mode >= VB_MED) {
      cout << "NNI step: " << nni_steps
           << " / Number of NNIs applied: " << numNNIs << endl;
    }
    ...
    // FOR TUNG: If you want to introduce this heuristic, please confirm with
    // reevaluation again.
    //        if (numNNIs > 1) {
    ...
    if (curScore >= nonConfNNIs.at(0).newloglh - params->loglh_epsilon) {
      ...
      // Diep: skip the 1NNI check if running weighted parsimony -mpars -cost
      // <cost_file>
      ...
      if (numNNIs == 1 && curScore < nonConfNNIs.at(0).newloglh - 1.0) {
        cout.precision(15);
        cout << "BUG: current logl=" << curScore << " < "
             << nonConfNNIs.at(0).newloglh << "(best NNI)" << endl;
        abort();
      }
      if (verbose_mode >= VB_MED) {
      ...
      restoreAllBrans();
      // This is important because after restoring the branch lengths, all
      // partial likelihood need to be cleared.
      clearAllPartialLH();

      // UPDATE: the following is not needed as clearAllPartialLH() is now also
      // defined for SuperTree BQM: This was missing: one should also clear all
      // subtrees of a supertree
      //            if (isSuperTree()) {
      //            	PhyloSuperTree *stree = (PhyloSuperTree*)this;
      //            	for (PhyloSuperTree::iterator it = stree->begin(); it !=
      //            stree->end(); it++) {
...
double IQTree::pllOptimizeNNI(int &totalNNICount, int &nniSteps,
  ...
  if ((globalParam->online_bootstrap == PLL_TRUE) &&
      (globalParam->gbo_replicates > 0) && (!globalParam->maximum_parsimony)) {
    pllInitUFBootData();
  }
  ...
  if (nniSteps == (MAX_NNI_STEPS + 1)) {
    cout << "WARNING: NNI search seems to run unusually too long and thus it "
            "was stopped!"
         << endl;
  }
  ...
  if (pllUFBootDataPtr == NULL) {
    ...
    if (params->online_bootstrap && params->gbo_replicates > 0) {
      ...
      pllUFBootDataPtr->treels_size =
          max_candidate_trees; // track size of treels_logl, treels_newick,
                               // treels_ptnlh
      ...
      // memset(pllUFBootDataPtr->treels_logl, 0, max_candidate_trees *
      // (sizeof(double)));
      ...
      if (!pllUFBootDataPtr->treels_newick)
        outError("Not enough dynamic memory!");
      memset(pllUFBootDataPtr->treels_newick, 0,
             max_candidate_trees * (sizeof(char *)));
...
void IQTree::optimizeBootTrees() {
  ...
  for (int sample = 0; sample < num_boot_rep; sample++) {
    ...
    //		out << sample << "\t" << boot_update_iter[sample] << "\t" <<
    // boot_trees[sample] << endl;
    bootstrap_aln = new Alignment;
    bootstrap_aln->modifyPatternFreq(*saved_aln_on_opt_btree,
    ...
    if (params->multiple_hits) { // process a few trees in
      ...
      for (IntegerSet::iterator it = boot_trees_parsimony[sample].begin();
        ...
        NodeVector taxai;
        getTaxa(taxai);
        for (NodeVector::iterator taxit = taxai.begin(); taxit != taxai.end();
             taxit++) {
    ...
    if (params->distinct_iter_top_boot >= 1 && (!params->multiple_hits)) {
      ...
      int best_boot_score = -INT_MAX;
      int id = 0;
      //			out << "sample#" << sample << ", boot_count = "
      //<< boot_counts[sample]
      //				<<", boot_threshold = " <<
      // boot_threshold[sample] << endl;
      ...
      if (do_concensus) {
        ...
        for (IntPairVector::iterator it =
                 boot_trees_parsimony_top[sample].begin();
        ...
        //				sample_cons =
        // computeConsensusTreeNoFileIO(sample_treels,
        ...
        computeConsensusTree(btree_file.c_str(), 0, params->tree_max_count,
                             ...
                             contree_file.c_str(), params->out_prefix,
      ...
      if (do_find_best) {
        ...
        for (IntPairVector::iterator it =
          ...
          //					out << it->first << "\t" <<
          // boot_trees_parsimony_top_iter[sample][id]
          //						<< "\t" << it->second <<
          //"\t";
          // Read the bootstrap tree
          stringstream str(tree);
    ...
    if ((!params->multiple_hits) &&
      ...
      //			out << "sample#" << sample << ", boot_count = "
      //<< boot_counts[sample] << endl; 			out <<
      // mit->second << "\t" << boot_logl[sample] << "\t";
      // Read the bootstrap tree
      stringstream str(tree);
      freeNode();
      readTree(str, rooted);
      ...
      //			// tmp, to-be-removed
      //#############################################
      // stringstream ostr1; 			printTree(ostr1, WT_SORT_TAXA);
    ...
    //		out << -boot_logl[sample] << endl; // to examine score after
    // refinement
  }
...
void IQTree::genNonconfNNIs() {
  for (vector<NNIMove>::iterator iterMove = plusNNIs.begin();
       iterMove != plusNNIs.end(); iterMove++) {
    bool choosen = true;
    for (vector<NNIMove>::iterator iterNextMove = nonConfNNIs.begin();
         iterNextMove != nonConfNNIs.end(); iterNextMove++) {
...
int IQTree::getDelete() const { return k_delete; }

void IQTree::setDelete(int _delete) { k_delete = _delete; }
...
void IQTree::evalNNIs(PhyloNode *node, PhyloNode *dad) {
  ...
  FOR_NEIGHBOR_IT(node, dad, it) { evalNNIs((PhyloNode *)(*it)->node, node); }
}

void IQTree::evalNNIs(map<string, Branch> brans) {
  for (map<string, Branch>::iterator it = brans.begin(); it != brans.end();
       it++) {
...
/**
 *  Currently not used, commented out to simplify the interface of
getBestNNIForBran void IQTree::evalNNIsSort(bool approx_nni) { if
(myMove.newloglh > curScore + params->loglh_epsilon) { if (myMove.newloglh >
curScore + params->loglh_epsilon) { addPositiveNNIMove(myMove);
        }
            addPositiveNNIMove(myMove);
        }
    ...
    for (i = 0; i < leafNum - 3; i++) {
        ...
        PhyloNeighbor *node12_it = (PhyloNeighbor*)
nodes1[i]->findNeighbor(nodes2[i]);
        //PhyloNeighbor *node21_it = (PhyloNeighbor*)
nodes2[i]->findNeighbor(nodes1[i]); int_branch.lh_contribution = cur_lh -
computeLikelihoodZeroBranch(node12_it, (PhyloNode*) nodes1[i]); if
(int_branch.lh_contribution < 0.0) int_branch.lh_contribution = 0.0; if
(int_branch.lh_contribution < fabs(nni_cutoff)) { int_branch.node1 =
(PhyloNode*) nodes1[i]; int_branch.node2 = (PhyloNode*) nodes2[i];
            int_branches.push_back(int_branch);
        }
    }
    std::sort(int_branches.begin(), int_branches.end(), int_branch_cmp);
    for (vector<IntBranchInfo>::iterator it = int_branches.begin(); it !=
int_branches.end(); it++) if (it->lh_contribution >= 0.0) // evaluate NNI if
branch contribution is big enough
                {
            NNIMove myMove = getBestNNIForBran(it->node1, it->node2, NULL,
approx_nni, it->lh_contribution); if (myMove.newloglh > curScore) {
                addPositiveNNIMove(myMove);
                if (!estimate_nni_cutoff)
                    for (vector<IntBranchInfo>::iterator it2 = it + 1; it2 !=
int_branches.end(); it2++) { if (it2->node1 == it->node1 || it2->node2 ==
it->node1 || it2->node1 == it->node2
                                || it2->node2 == it->node2)
                            it2->lh_contribution = -1.0; // do not evaluate this
branch later on
                    }
            }
            ...
            PhyloNeighbor *node12_it = (PhyloNeighbor*)
node1->findNeighbor(node2); PhyloNeighbor *node21_it = (PhyloNeighbor*)
node2->findNeighbor(node1); double stored_len = node12_it->length; curScore =
optimizeOneBranch(node1, node2, false); string key(""); if (node1->id <
node2->id) { key += convertIntToString(node1->id) + "->" +
convertIntToString(node2->id); } else { key += convertIntToString(node2->id) +
"->" + convertIntToString(node1->id);
            }

            optBrans.insert(mapString2Double::value_type(key,
node12_it->length)); node12_it->length = stored_len; node21_it->length =
stored_len;
        }
}
*/
...
void IQTree::estimateNNICutoff(Params *params) {
  ...
  for (i = 0; i < nni_info.size(); i++) {
    ...
    if (verbose_mode >= VB_MED)
      cout << i << ": " << lh_score[0] << " " << lh_score[1] << " "
           << lh_score[2] << " " << lh_score[3] << endl;
  }
 ...
 * 			params->spr_parsimony && !on_ratchet_hclimb1 &&
 * !params->hclimb1_nni means the tree is already in IQTREE data structure
 */
void IQTree::saveCurrentTree(double cur_logl) {
  ...
  if (params->maximum_parsimony && on_ratchet_hclimb1 && reps_segments != -1) {
    ...
    for (; segment_id < reps_segments; segment_id++) {
      for (; ptn < segment_upper[segment_id]; ptn += VCSIZE_USHORT)
        vc_score = VectorClassUShort().load_a(&_pattern_pars[ptn]) *
  ...
  if (it != treels.end()) { // already in treels
    ...
    if (cur_logl <= treels_logl[it->second] + 1e-4) {
      if (cur_logl < treels_logl[it->second] - 5.0)
        if (verbose_mode >= VB_MED)
          cout << "Current lh " << cur_logl << " is much worse than expected "
               << treels_logl[it->second] << endl;
      return;
    }
    if (verbose_mode >= VB_MAX)
      cout << "Updated logl " << treels_logl[it->second] << " to " << cur_logl
           << endl;
    treels_logl[it->second] = cur_logl;
    if (save_all_br_lens) {
      ostr.seekp(ios::beg);
      printTree(ostr, WT_TAXON_ID | WT_SORT_TAXA | WT_BR_LEN | WT_BR_SCALE |
                          WT_BR_LEN_ROUNDING);
    ...
    if (logl_cutoff != 0.0 &&
        cur_logl <= logl_cutoff - 1e-4) // Diep: change + to - to avoid cases
                                        // similar to Goloboff example
      return;
  ...
  if (params->maximum_parsimony) {
    if (params->spr_parsimony && !(params->ratchet_iter >= 0 &&
      ...
      if (!on_ratchet_hclimb1 && test_pars != -int(cur_logl))
        outError("WRONG pllComputeSiteParsimony: sum of site parsimony is "
                 "different from alignment parsimony");
    }

    if (!params->auto_vectorize && reps_segments > 1 &&
        boot_samples_pars_remain_bounds[0] == NULL) { // do this once
      double starts = getCPUTime();
      cout << "NOTE: REPS computation is segmented into " << reps_segments
           << " parts for speed..." << endl;
  ...
  if (!params->maximum_parsimony && boot_samples.empty()) {
    ...
    int ptn;
    int updated = 0;
    int nsamples = (params->maximum_parsimony) ? boot_samples_pars.size()
                                               : boot_samples.size();
    ...
    for (int sample = 0; sample < nsamples; sample++) {
      ...
      if (params->maximum_parsimony) {
          ...
          for (; segment_id < reps_segments; segment_id++) {
            for (; ptn < segment_upper[segment_id]; ptn += VCSIZE_USHORT) {
              if (params->do_first_rell && ptn >= max_nptn)
                break;
              vc_rell = VectorClassUShort().load_a(&_pattern_pars[ptn]) *
                            VectorClassUShort().load_a(&boot_sample[ptn]) +
                        vc_rell;
            }
            ...
            if ((!skipped) && (reps_segments > 1) &&
                ...
                (segment_id < reps_segments - 1)) {
              int reps_total =
                  res + boot_samples_pars_remain_bounds[sample][segment_id];
              if ((double)(-reps_total) <
                  boot_logl[sample] - params->ufboot_epsilon) {
                //								cout
                //<< "Current best of sample " << sample << " = " <<
                // int(-boot_logl[sample]) << endl;
                // cout << "REPS at segment " << segment_id << " = " << res +
                // boot_samples_pars_remain_bounds[sample][segment_id] << endl;
                //								cout
                //<< "NOTE: estimated value for boot sample parsimony exceeds
                // its
                // current best."
                //										<<
                //"Skip on sample " << sample << " at site " <<
                // segment_upper[segment_id] << endl;
                skipped = true;
                break;
              }
            }
        ...
        // TODO: The following parallel is not very efficient, should wrap the
        // above loop
        //#ifdef _OPENMP
        ...
        // boot_samples[sample]; BootValType rellll = 0.0;
        // for (ptn = 0; ptn < nptn; ptn++)
        // rellll
        // += pattern_lh[ptn]
        // * boot_sample[ptn]; 					rell =
        //(double)rellll; 				} else {
        ...
        for (ptn = 0; ptn < maxptn; ptn += VCSIZE_FLOAT)
          vc_rell =
              mul_add(VectorClassFloat().load_a(&pattern_lh[ptn]),
        ...
        for (ptn = 0; ptn < maxptn; ptn += VCSIZE_MASTER)
          vc_rell =
              mul_add(VectorClassMaster().load_a(&pattern_lh[ptn]),
      ...
      */
      ...
      if (params->multiple_hits) {
        ...
        if (rell >= boot_logl[sample] && !params->store_top_boot_trees) {
          if (tree_str == "") {
            if (params->spr_parsimony &&
                !(params->ratchet_iter >= 0 && on_ratchet_hclimb1 &&
                  params->hclimb1_nni)) {
              pllTreeToNewick(pllInst->tree_string, pllInst, pllPartitions,
                              pllInst->start->back, PLL_TRUE, PLL_TRUE, 0, 0, 0,
                              PLL_SUMMARIZE_LH, 0, 0);
            ...
            if (it != treels.end()) {
              ...
              tree_index = treels_logl.size() -
                           1; // old statement is wrong: treels.size();
              treels[tree_str] = tree_index;
            }
          }
          ...
          // Change boot_trees_parsimony[sample] type from IntVector to
          // IntegerSet We can keep using the IntVec and check whether
          // tree_index == treels_logl.size() - 1
          if (boot_trees_parsimony[sample].find(tree_index) ==
              boot_trees_parsimony[sample].end()) {
        ...
        if (params->store_top_boot_trees) {
          if (boot_trees_parsimony_top[sample].size() <
                  params->store_top_boot_trees ||
            ...
            if (tree_index == treels_logl.size() - 1) {
              ...
              if (count < params->store_top_boot_trees) {
                ...
                boot_trees_parsimony_top[sample].insert(
                    it, std::make_pair(tree_index, rell));
                boot_threshold[sample] = (boot_threshold[sample] < rell)
                                             ? boot_threshold[sample]
                                             : rell;
              } else if (count == params->store_top_boot_trees &&
                         rell > boot_threshold[sample]) {
                ...
                boot_trees_parsimony_top[sample].insert(
                    it, std::make_pair(tree_index, rell));
                boot_threshold[sample] =
                    boot_trees_parsimony_top[sample]
                                            [params->store_top_boot_trees - 1]
                                                .second;
              }
              updated++;
            }
      ...
      if (params->distinct_iter_top_boot >= 1 && !params->multiple_hits) {
        ...
        if (rell > boot_threshold[sample] ||
            (rell == boot_threshold[sample] &&
             random_double() <= (params->distinct_iter_top_boot * 1.0 /
                                 (boot_counts[sample])))) {
          //				if (rell >= boot_threshold[sample]){
          ...
          boot_trees[sample] =
              tree_index; // this will be lastly updated in refinement step
          boot_logl[sample] = max(boot_logl[sample], rell);

          int t = min((int)params->distinct_iter_top_boot,
                      (int)boot_trees_parsimony_top_iter[sample].size());
          int c;
          ...
          // if top list is not full and iteration representative does not
          // exist, ADD
          if (c == t & t < params->distinct_iter_top_boot) {
            boot_trees_parsimony_top_iter[sample].push_back(curIt);
            boot_trees_parsimony_top[sample].push_back(
                std::make_pair(tree_index, rell));
          }
          ...
          // setting boot_threshold[sample] to be the worst in
          // boot_trees_parsimony_top[sample]
          boot_threshold[sample] = boot_trees_parsimony_top[sample][0].second;
          for (int d = 1; d < boot_trees_parsimony_top[sample].size(); d++) {
            if (boot_trees_parsimony_top[sample][d].second <
                boot_threshold[sample]) {
              boot_threshold[sample] =
                  boot_trees_parsimony_top[sample][d].second;
            }
          }
...
void IQTree::pllComputeRellRemainBound(int nunit) {
  ...
  for (int i = 0; i < nunit; i++) {
    if (cost_matrix == NULL) {
      // unweighted case
      int pll_min = pllCalcMinParsScorePattern(
          pllInst, pllPartitions->partitionData[0]->dataType, i);
      int cur_min;
      if (params->sort_alignment)
      ...
      //			if(i == 253) cout << "pll_min = " << pll_min <<
      //", cur_min = " << cur_min << endl;
    } else {
      // weighted case
...
void IQTree::summarizeBootstrap(Params &params, MTreeSet &trees) {
  ...
  int max_tree_id =
      max_element(trees.tree_weights.begin(), trees.tree_weights.end()) -
      trees.tree_weights.begin();
  if (verbose_mode >= VB_MED) {
    cout << "max_tree_id = " << max_tree_id + 1
  ...
  if (!boot_splits.empty()) {
    ...
    if (computeBootstrapCorrelation() < params.min_correlation)
      cout << "WARNING: bootstrap analysis did not converge. You should rerun "
              "with higher number of iterations (-nm option)"
           << endl;
  }
...
void IQTree::writeUFBootTrees(Params &params, StrVector &removed_seqs,
                              StrVector &twin_seqs) {
  MTreeSet trees;
...
void IQTree::summarizeBootstrapParsimony(Params &params) {
        setRootNode(params.root);
        if (verbose_mode >= VB_MED)
                cout << "Summarizing from " << treels.size() << " candidate
trees..." << endl;
//    MTreeSet btrees;
...
//		}
//		string bsample_tree = computeConsensusTreeNoFileIO(treels,
...
//    summarizeBootstrap(params, btrees);
    ...
    for (sample = 0; sample < boot_trees_parsimony.size(); sample++){
            tree_weights.resize(boot_trees_parsimony[sample].size(), 1);
                string bsample_tree =
computeConsensusTreeNoFileIO(boot_trees_ls_parsimony[sample], tree_weights,
...
void IQTree::summarizeBootstrapParsimonyTop(Params &params) {
  ...
  for (int sample = 0; sample < boot_trees_parsimony_top.size(); sample++) {
    ...
    for (int i = 0; i < boot_trees_parsimony_top[sample].size(); i++) {
      ...
      //			fout << "Tree # " <<
      // boot_trees_parsimony[sample][i] << " scored " <<
      // boot_trees_parsimony_score[sample][i] << endl;
    }
  }
  //    fout.close();
  btrees.init(treels, rooted, btree_weights);
...
params->tree_max_count,
    ...
    btrees.convertSplits(taxname, sg, hash_ss, SW_COUNT, -1, false); // do not
sort taxa
}
*/
...
void IQTree::summarizeBootstrapParsimonyTop(SplitGraph &sg) {
  ...
  for (int sample = 0; sample < boot_trees_parsimony_top.size(); sample++) {
    ...
    for (int i = 0; i < boot_trees_parsimony_top[sample].size(); i++) {
      btree_weights[boot_trees_parsimony_top[sample][i].first] += 1;
      //			cout <<
      // boot_trees_parsimony_top[sample][i].second << ", ";
    }
    //		cout << endl;
  }
  btrees.init(treels, rooted, btree_weights);
...
void IQTree::addPositiveNNIMove(NNIMove myMove) { plusNNIs.push_back(myMove); }
...
void IQTree::printResultTree(string suffix) {
  ...
  if (suffix.compare("") != 0) {
    ...
    //        printTree(iter_tree_name.c_str(), WT_BR_LEN |
    //        WT_BR_LEN_FIXED_WIDTH | WT_SORT_TAXA | WT_NEWLINE); // for ML
    printTree(iter_tree_name.c_str(),
              WT_SORT_TAXA | WT_NEWLINE); // Diep: for MP
  } else {
...
void IQTree::printPhylolibTree(const char *suffix) {
  pllTreeToNewick(pllInst->tree_string, pllInst, pllPartitions,
                  pllInst->start->back, PLL_TRUE, 1, 0, 0, 0, PLL_SUMMARIZE_LH,
                  0, 0);
