...
class Linear {
  public:
    ...
    double getValue(double x) { return m_a + m_b * x; }

    //! Returns the slope of the regression line

    double getSlope() { return m_b; }

    //! Returns the intercept on the Y axis of the regression line

    double getIntercept() { return m_a; }

    //! Returns the linear regression coefficient

    double getCoefficient() { return m_coeff; }
...
}; inline double *operator[] (int i) {	return value + (i*cols); }; virtual
~DoubleMatrix(); void setZero();
};
 */
...
enum RunMode {
    ...
    LINEAR_PROGRAMMING,
    STATS //, GBO, MPRO
}; // STATS and GBO added by MA (STATS for some statistics on tree, GBO = guided
   // 'bootstrap'
...
/**
        when writing tree:
                ...
                WT_BR_SCALE - for draw tree, draw the branch proportional to its
   length WT_SORT_TAXA - sort the taxa s.t. subtrees with least taxon ID come
   first WT_APPEND    - append the output file WT_NEWLINE   - print a newline
   after WT_BR_LEN_FIXED_WIDTH - print branch length in fixed number format
 */
 ...
 *  TOPO_UPDATE_LV: the same as above but the partial likelihoods are update in
 * addition NNI1: optimize the central branch after changing the tree topology
 *  NNI5: optimized the 5 affected branches after changing the tree topology
 */
enum NNI_Type { TOPO_ONLY, TOPO_UPDATE_LV, NNI1, NNI5 };
...
const int RF_TWO_TREE_SETS_EXTENDED =
    4; // work for trees with non-equal taxon sets
...
const int SW_AVG_PRESENT =
    4; // take the split weight average over all trees that the split is present
 ...
 */
enum InputType { IN_NEWICK, IN_NEXUS, IN_FASTA, IN_PHYLIP, IN_OTHER };
...
enum AlnFormat { ALN_PHYLIP, ALN_FASTA };
...
enum IQP_ASSESS_QUARTET { IQP_DISTANCE, IQP_PARSIMONY, IQP_BOOTSTRAP };
...
enum START_TREE_TYPE { STT_BIONJ, STT_PARSIMONY, STT_PLL_PARSIMONY };

const int MCAT_LOG =
    1; // categorize by log(rate) for Meyer & von Haeseler model
const int MCAT_MEAN = 2; // take the mean of rates for each category for Meyer &
                         // von Haeseler model
const int MCAT_PATTERN = 4; // categorize site-patterns instead of sites for
                            // Meyer & von Haeseler model

const double MAX_GENETIC_DIST = 9.0;

struct NNIInfo {
    double lh_score[4]; // tree log-likelihood of zero-branch, current tree, NNI
                        // tree 1, NNI tree 2
    double br_len[4]; // length of current branch, optimized branch, NNI branch
                      // 1, NNI branch 2
    int nni_round;
    int iqpnni_iteration;
};

enum LikelihoodKernel { LK_NORMAL, LK_SSE, LK_EIGEN, LK_EIGEN_SSE };

enum LhMemSave { LM_DETECT, LM_ALL_BRANCH, LM_PER_NODE };
...
struct Params {
    ...
    /**
     * TBR swap branch when connect 2 inserted branches
     */
    int tbr_insert_nni;
    ...
    /**
     * 		defines the relation between edge lengths in supertree and
     * subtrees 0 (NULL) for separate edge length (default) 'p' for proportional
     * edge length 'j' for joint edge length
     */
    char partition_type;

    /** remove all-gap sequences in partition model to account for terrace
     * default: TRUE */
    ...
    /**
            file containing site likelihood as input for 'guided bootstrap'
       (added by MA)
     */
    char *siteLL_file;

    /**
            alignment where the gappy patterns will be superimposed into the
       input alignment
     */
    char *gap_masked_aln;
    ...
    /**
            TRUE to compute the observed distances instead of Juke-Cantor
    ...
    /**
            TRUE if one wants to calculate the PD gain matrix in terms of
       delta_k^j = pd(PD_k \/ {j}) - pd_k
     */
    bool calc_pdgain;
    ...
    /**
            set the TRUE if want to find the minimal PD set, instead of the
       default maximal PD set
     */
    bool find_pd_min;
    ...
    /**
            maximum number of trees to consider (for e.g. consensus tree
       construction)
     */
    int tree_max_count;

    /**
        threshold of split frequency, splits appear less than threshold will be
       discarded
     */
    double split_threshold;

    /**
            threshold of split weight, splits with weight less than or equal to
       threshold will be discarded
     */
    double split_weight_threshold;

    /**
            Way to summarize split weight in the consensus tree or network:
       SW_SUM, SW_AVG_ALL, or SW_AVG_PRESENT
     */
    ...
    /**
            TRUE to store transition matrix into a hash table for computation
       efficiency
     */
    ...
    /**
            TRUE to use median rate for discrete categories, FALSE to use mean
       rate instead
     */
    bool gamma_median;
    ...
    /** TRUE to optimize all model and rate parameters jointly by BFGS, default:
     * FALSE */
    ...
    /** bootstrap specification of the form "l1:b1,l2:b2,...,lk:bk"
        to randomly draw b1 sites from the first l1 sites, etc. Note that
       l1+l2+...+lk must equal m, where m is the alignment length. Otherwise, an
       error will occur. The default bootstrap_spec == NULL, a standard
    ...
    */
    ...
    /**
            Robinson-Foulds distance computation mode: RF_ADJACENT PAIR,
       RF_ALL_PAIR
     */
    int rf_dist_mode;
    ...
    /**
            Percentage threshold to accept a branch of the approximate
       likelihood ratio test (aLRT) with SH-like interpretation. See Guindon et
       al. (2010) Syst. Biol. for details. Default: 90%.
     */
    int aLRT_threshold;
    ...
    /**
            number of replicates for local bootstrap probabilities method of
       Adachi & Hasegawa (1996) in MOLPHY
     */
    ...
    /**
            next-generation sequencing input file containing mapped reads to the
       reference genome
     */
    ...
    /**
            rank to ingore, e.g., "no rank", branch length to such node will be
       set to zero
     */
    ...
    /**
            eco_dag_file - contains the food web in matrix form (n species, nxn
       matrix), 0 for no connection, 1 for predation of j predator on i prey
    */
    char *eco_dag_file;

    /**
                eco_detail_file - contains IDs of species present in the final
       set and/or species absent in the TREE or SPLIT system, but present in the
       food web
        */
    ...
    /*
                diet - percent of species diet to be preserved for species
       survival
        */
    int diet_min;
    int diet_max;
    ...
    /*
                eco_run - run number, used when random branch length is assigned
       to the edges of an input tree
        */
    int eco_run;

    /*
                eco_weighted - indicates whether to treat the food web as
    ...
    /**
            TRUE to use ELW method of Strimmer & Rambaut for new bootstrap,
       FALSE otherwise
     */
    bool use_elw_method;

    /**
            TRUE to weight each bootstrap sample by its probability, FALSE
       otherwise
     */
    bool use_weighted_bootstrap;

    /**
            TRUE to use the single ML tree per bootstrap, FALSE to include
       several sup-optima
     */
    ...
    /** NEW: TRUE to update bootstrap trees during the search (do not store
       treels_ptnlh). FALSE to call runGuidedBootstrap() at the end */
    ...
    /**
            logl difference with zero-branch tree, to cutoff before evaluating
       NNI
     */
    double nni_cutoff;
    ...
    /** TRUE to do approximate NNIs with approximate branch lengths before a
     * normal NNI */
    ...
    /** file containing state-frequencies per site for site-specific state
     * frequency model each line has n+1 entries (n=number of states): site_ID
     * state1_freq state2_freq ... staten_freq where site_ID is from 1 to m
     * (m=number of sites)
     */
    char *site_freq_file;
    ...
    int store_top_boot_trees; // number of top trees to store per boot sample
                              // (e.g. top 10, top 5)
    ...
    /*
     * Diep: option for computing parsimony RELL with compiler's automatic
     * vectorization
     */
    bool auto_vectorize;

    /*
     * Diep: option for sorting alignment by decreasing (score * freq) of
     * pattern
     */
    bool sort_alignment;
    ...
    /*
     * Diep: to turn on the option for the 1st hill-climbing in ratchet
     * iteration by NNI
     */
    bool hclimb1_nni;

    /*
     * Diep: to turn off saveCurrentTree() during the 1st hill-climbing in
     * ratchet iteration
     */
    bool no_hclimb1_bb;
    ...
    /*
     * Diep: to not collect any tree during bootstrap, applicable only if used
     * with optimize_boot_trees -opt_btree -save_trees_off means not running any
     * saveCurrentTree() but running optimizeBootTrees()
     */
    bool save_trees_off;
     ...
     * 		- use 100 best trees found in the iteration to calculate
     * logl_cutoff
     */
     ...
     */
    int distinct_iter_top_boot; // top k boot trees picked from distinct
                                // iterations
    ...
    /** TRUE (default) to ignore identical sequences and add them back at the
     * end
     */
 ...
 */
inline bool controlchar(char ch) { return ch <= 32; }
...
const char ERR_CONFLICT_ROOT[] =
    "Tree is already rooted, -o <taxon> is not allowed.";
...
const char ERR_FEW_SPLITS[] =
    "Number of splits must be at least equal to the number of taxa";
...
const char ERR_TOO_SMALL_K[] =
    "Size of PD-set must be at least the size of initial set.";
const char ERR_NO_BUDGET[] = "Total budget is not specified or less than zero.";
const char ERR_TOO_SMALL_BUDGET[] =
    "Not enough budget to conserve the inital set of taxa.";
...
/**
        ...
        @param filename file name
        @return TRUE if agree to overwrite an existing file, or simply file does
   not exist
 */
...
/**
 * generate 2 different random integer numbers smaller than a specific integer
 * threshold
...
template <class T> void my_random_shuffle(T first, T last) {
    int n = last - first;
    for (int i = n - 1; i > 0; --i) {
 ...
 */
template <class T> void sort_index(T *first, T *last, int *index) {
    T *x;
    int i;
 ...
 */
void summarizeHeader(ostream &out, Params &params, bool budget_constraint,
